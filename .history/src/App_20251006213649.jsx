                ...
                <>
                  <>
                    ...
                    {movies.length > 0 && <MovieSection 
                      ...
                      description="Tuyển tập anime mới nhất, đa dạng thể loại - Hơn 50 phim anime chất lượng cao"
                      movies={(() => {
                        // Tìm kiếm anime với nhiều tiêu chí hơn
                        const animeMovies = filterMoviesByTheme(movies, 'anime');
                        const japaneseMovies = filterMoviesByTheme(movies, 'japanese');
                        
                        // Tìm phim hoạt hình/anime với nhiều tiêu chí
                        const animationMovies = movies.filter(movie => {
                          ...
                          return category.includes('hoạt hình') || 
                                 category.includes('animation') ||
                                 category.includes('anime') ||
                                 movieName.includes('anime') ||
                                 originName.includes('anime') ||
                                 movieName.includes('hoạt hình') ||
                                 originName.includes('animation');
                        });
                        
                        // Tìm phim Nhật Bản (thường có nhiều anime)
                        const japanMovies = movies.filter(movie => {
                        ...
                        let allAnimeMovies = [...animeMovies, ...japaneseMovies, ...animationMovies, ...japanMovies];
                        
                        // Loại bỏ trùng lặp dựa trên slug
                        const uniqueAnimeMovies = [];
                        const seenSlugs = new Set();
                        
                        for (const movie of allAnimeMovies) {
                          if (movie.slug && !seenSlugs.has(movie.slug)) {
                            seenSlugs.add(movie.slug);
                            uniqueAnimeMovies.push(movie);
                          }
                        }
                        
                        console.log(`Found ${uniqueAnimeMovies.length} unique anime movies`);
                        
                        // Ưu tiên animeMovies, sau đó japaneseMovies, cuối cùng là animationMovies
                        if (animeMovies.length > 0) {
                          return animeMovies.slice(0, 50);
                        } else if (japaneseMovies.length > 0) {
                          return japaneseMovies.slice(0, 50);
                        } else if (uniqueAnimeMovies.length > 0) {
                          return uniqueAnimeMovies.slice(0, 50);
                        } else {
                          return animationMovies.slice(0, 50);
                        }
                      })()}
                    />}
                    
                    {/* Anime Nhật Bản Đặc Sắc */}
                    {movies.length > 0 && <MovieSection 
                      title="Anime Nhật Bản Đặc Sắc"
                      description="Tuyển tập anime Nhật Bản chất lượng cao và nổi tiếng"
                      movies={(() => {
                        // Tìm phim Nhật Bản với thể loại anime/hoạt hình
                        const japanAnimeMovies = movies.filter(movie => {
                          const country = movie.country?.map(c => c.name?.toLowerCase()).join(' ') || '';
                          const category = movie.category?.map(c => c.name?.toLowerCase()).join(' ') || '';
                          const movieName = (movie.name || '').toLowerCase();
                          
                          const isJapanese = country.includes('nhật bản') || 
                                           country.includes('japan') || 
                                           country.includes('japanese');
                          
                          const isAnime = category.includes('hoạt hình') ||
                                         category.includes('animation') ||
                                         category.includes('anime') ||
                                         movieName.includes('anime') ||
                                         movieName.includes('hoạt hình');
                        ...
                        // Nếu không tìm thấy anime Nhật, tìm phim Nhật Bản nói chung
                        if (japanAnimeMovies.length === 0) {
                          const japanMovies = movies.filter(movie => {
                            const country = movie.country?.map(c => c.name?.toLowerCase()).join(' ') || '';
                            return country.includes('nhật bản') || 
                                   country.includes('japan') || 
                          ...
                          return japanMovies.slice(0, 30);
                        }
                        
                        return japanAnimeMovies.slice(0, 30);
                      })()}
                    />}
                    ...
                    {movies.length > 0 && <MovieSection 
                      ...
                      description="Bộ sưu tập anime và manga nổi tiếng từ Nhật Bản"
                      movies={(() => {
                        // Tìm tất cả phim liên quan đến anime, manga, hoạt hình
                        const animeCollection = movies.filter(movie => {
                          const category = movie.category?.map(c => c.name?.toLowerCase()).join(' ') || '';
                          const movieName = (movie.name || '').toLowerCase();
                          const originName = (movie.origin_name || '').toLowerCase();
                          const country = movie.country?.map(c => c.name?.toLowerCase()).join(' ') || '';
                          
                          // Kiểm tra nhiều tiêu chí anime/manga
                          const isAnime = category.includes('hoạt hình') ||
                                         category.includes('animation') ||
                                         category.includes('anime') ||
                                         movieName.includes('anime') ||
                                         originName.includes('anime') ||
                                         movieName.includes('manga') ||
                                         originName.includes('manga');
                          
                          const isJapanese = country.includes('nhật bản') || 
                                           country.includes('japan') || 
                                           country.includes('japanese');
                          
                          // Ưu tiên anime Nhật Bản, nhưng cũng lấy anime từ các nước khác
                          return isAnime || (isJapanese && (movieName.includes('hoạt hình') || category.includes('hoạt hình')));
                        });
                        
                        // Sắp xếp theo năm (mới nhất trước)
                        animeCollection.sort((a, b) => {
                          const yearA = parseInt(a.year) || 0;
                          const yearB = parseInt(b.year) || 0;
                          return yearB - yearA;
                        });
                        
                        console.log(`Found ${animeCollection.length} anime collection movies`);
                        return animeCollection.slice(0, 40);
                      })()}
                    />}
