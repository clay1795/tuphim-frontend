...
const AuthProvider = ({ children }) => {
  ...
  const removeFromFavorites = async (movieId) => {
    try {
      console.log('🔧 AuthContext: removeFromFavorites called with movieId:', movieId);
      ...
      if (!response.ok) {
        console.error('🔧 AuthContext: Request failed:', response.status, data);
        return { success: false, message: data.message || 'Xóa khỏi yêu thích thất bại' };
      }

      return { success: true, data: data.data };
    } catch (error) {
      console.error('🔧 AuthContext: Remove from favorites error:', error);
      return { success: false, message: 'Có lỗi xảy ra' };
    }
  };
