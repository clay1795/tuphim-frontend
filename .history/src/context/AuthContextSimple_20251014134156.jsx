...
const AuthProvider = ({ children }) => {
  ...
  const updateProfile = async (profileData) => {
    try {
      ...
      if (data.success && data.data) {
        ...
        // Update username in all comments if fullName changed
        if (profileData.fullName && profileData.fullName !== user?.fullName) {
          try {
            await fetch(`${getApiBaseUrl()}/comments/update-username/${user._id}`, {
              method: 'PUT',
              headers: getAuthHeaders(),
              body: JSON.stringify({ newUsername: profileData.fullName }),
            });
            console.log('✅ Updated username in all comments');
          } catch (commentError) {
            console.warn('⚠️ Failed to update comments:', commentError);
            // Don't fail the profile update if comment update fails
          }
        }
  ...
  const addToHistory = async (movieData) => {
    try {
      const response = await fetch(`${getApiBaseUrl()}/users/history', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify(movieData),
      });
  ...
  const getUserStats = async () => {
    try {
      const response = await fetch(`${getApiBaseUrl()}/users/stats', {
        method: 'GET',
        headers: getAuthHeaders(),
      });
