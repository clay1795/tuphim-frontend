...
const MovieProvider = ({ children }) => {
  ...
  const fetchMovieDetail = useCallback(async (movieSlug) => {
    setLoading(true);
    try {
      console.log('Loading movie detail for slug:', movieSlug);
      const response = await movieApi.getMovieDetail(movieSlug);
      console.log('Movie detail API response:', response);
      ...
      if (response && response.success && response.data) {
        ...
        // Nếu API trả về lỗi, throw error để vào catch block
        throw new Error(response.message || 'API returned error');
      } else {
        movieData = response;
      }
      
      console.log('Processed movie detail data:', movieData);
      setMovieDetail(movieData);
      return movieData;
    } catch (error) {
      console.error('Error loading movie detail:', error);
      ...
      const fallbackData = {
        _id: movieSlug,
        slug: movieSlug,
        name: movieSlug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
        title: movieSlug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
        ...
        country: [{ name: 'USA' }],
        content: 'Phim không khả dụng. Vui lòng thử lại sau.',
        embedUrl: `https://goatembed.com/${movieSlug}?version=1`,
        episodes: [
          {
            episode_name: 'Tập 1',
            server_data: [
              {
                link_m3u8: `https://goatembed.com/${movieSlug}?version=1`
              }
            ]
          }
        ]
  ...
  return (
    <MovieContext.Provider value={{ 
      ...
      movieDetail,
      loading,
      error: null, // Có thể thêm error state nếu cần
      fetchMovieDetail
    }}>
      {children}
...
const useMovieDetailContext = () => {
  const context = useContext(MovieContext);
  if (!context) {
    throw new Error('useMovieDetailContext must be used within a MovieProvider');
  }
  return context;
};

export { MovieProvider, MovieContext, useMovieDetailContext };
