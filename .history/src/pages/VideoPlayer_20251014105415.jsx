...
const EpisodeSelector = ({ episodes, currentEpisode, currentServer, onEpisodeChange, maxVisible = 12 }) => {
      ...
      <div className="flex flex-wrap gap-2">
        {episodes.map((episode, episodeIndex) => (
          <button
            ...
            className={`px-3 py-1 rounded-full text-sm ring-1 ring-[#44475A] transition-colors ${
  ...
  const currentPage = Math.floor(currentEpisode / maxVisible) + 1;
  const totalPages = Math.ceil(totalEpisodes / maxVisible);
...
const VideoPlayer = () => {
  ...
  // Update video URL when episode or audio type changes
  useEffect(() => {
    if (selectedEpisode) {
      let newUrl = null;
  ...
  }, [showEpisodeList, selectedServer, episodes]);
  ...
  const navigateToEpisode = useCallback((direction) => {
    ...
    if (newEpisodeIndex >= 0 && newEpisodeIndex < selectedServer.server_data.length) {
      ...
      toast.success(`Chuyển sang tập ${newEpisodeIndex + 1}`);
    } else {
  ...
  const handleEpisodeChange = useCallback((newServerIndex, newEpisodeIndex) => {
    ...
    setIsPlaying(false);
    // Reset to subtitle if dub is not available for new episode
    const newEpisode = episodes[newServerIndex]?.server_data?.[newEpisodeIndex];
    if (newEpisode && !newEpisode.link_dub && audioType === "dub") {
  ...
  const setAudio = (type) => {
    ...
    if (type === "dub" && !hasDubVersion) {
      toast.error("Phim này không có bản thuyết minh");
      return;
    }
    ...
    toast.success(type === "subtitle" ? "Đã chuyển sang Phụ đề" : "Đã chuyển sang Thuyết minh");
  };
