          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6">
            ...
            <div className="lg:col-span-1">
              <div className="relative max-w-[200px] mx-auto lg:mx-0">
                <img
                  src={movie.poster_url || movie.thumb_url || '/api/placeholder/200/300'}
                  alt={movie.name}
                  className="w-full h-auto rounded-lg shadow-lg border border-[#272A3A]"
                  onError={(e) => {
                ...
                {/* Quality Badge */}
                {movie.quality && (
                  <div className="absolute top-2 right-2 bg-[#FFC94A] text-black px-2 py-1 rounded text-xs font-bold">
                    {movie.quality}
                  </div>
                )}
              </div>
            ...
            <div className="lg:col-span-3 space-y-4">
              {/* Title Section */}
              <div>
                <h2 className="text-2xl font-bold text-[#F5F5F5] mb-2">{movie.name}</h2>
                {movie.origin_name && (
                  <p className="text-[#F5F5F5]/70 text-lg">{movie.origin_name}</p>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[#F5F5F5] font-semibold">{movie.rating || '8.5'}</span>
                </div>
                {movie.age && (
                  <span className="bg-[#FFC94A] text-black px-2 py-1 rounded text-xs font-bold">
                    {movie.age}
                  </span>
                )}
                <span className="text-[#F5F5F5]/70">{movie.year || '2024'}</span>
                {selectedServer && selectedServer.server_data && (
                  <span className="text-[#F5F5F5]/70">
                    {episodes.length > 1 ? `Phần ${episodes.indexOf(selectedServer) + 1}` : ''} - {selectedServer.server_data.length} Tập
                  </span>
                )}
              </div>
              ...
              {movie.category && movie.category.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {movie.category.slice(0, 3).map((genre, index) => (
                    <span
                      key={index}
                      className="bg-[#8B5CF6] text-white px-3 py-1 rounded-full text-sm font-medium"
                    >
                      {genre.name}
                    </span>
                  ))}
              ...
              {/* Broadcast Schedule */}
              {movie.time && (
                <div className="bg-[#8B5CF6] text-white px-4 py-2 rounded-full text-sm flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                  <span>Lịch phát sóng: {movie.time}</span>
                </div>
              )}

              {/* Movie Content/Synopsis */}
              <div>
                <h3 className="text-[#F5F5F5] font-bold text-lg mb-3">Nội dung phim</h3>
                <p className="text-[#F5F5F5]/90 leading-relaxed">
                  {movie.content || 'Một câu chuyện hấp dẫn và thú vị với những tình tiết bất ngờ và nhân vật đầy sức sống. Hãy cùng theo dõi hành trình đầy cảm xúc của các nhân vật trong bộ phim này.'}
                </p>
              </div>
