...
const VideoPlayer = () => {
  ...
  return (
    <div className="min-h-screen bg-black relative">
      ...
      {/* Main Video Player - PhimBro Style */}
      <div 
        className="relative w-full h-screen bg-black cursor-pointer group"
      ...
      >
        ...
        <div className="relative w-full h-full">
          {selectedEpisode.link_m3u8 || selectedEpisode.link_embed ? (
            <div className="relative w-full h-full">
              ...
              {/* Play/Pause Overlay - PhimBro Style */}
              {!isPlaying && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/30">
                  <button 
                    onClick={handlePlayPause}
                    className="w-16 h-16 bg-white/90 hover:bg-white rounded-full flex items-center justify-center transition-all duration-300 transform hover:scale-110 shadow-2xl"
                  >
        ...
        {/* Video Controls Overlay - PhimBro Style */}
        {showControls && (
            ...
            <div className="flex items-center justify-between text-white max-w-6xl mx-auto">
              ...
              <div className="flex items-center space-x-3">
                ...
                <button 
                  ...
                  className="w-9 h-9 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center transition-all duration-200"
                >
              ...
              {/* Center - Progress Bar - PhimBro Style */}
              <div className="flex-1 mx-6">
                <div className="flex items-center space-x-3">
                  <span className="text-xs font-medium text-gray-300 min-w-[40px]">{formatTime(currentTime)}</span>
                  <div 
                    className="flex-1 bg-gray-700 rounded-full h-1 cursor-pointer group"
                    onClick={handleProgressClick}
                  >
                    <div
                      className="bg-blue-500 h-1 rounded-full transition-all duration-200 group-hover:h-1.5"
                      style={{
                        width: duration > 0 ? `${(currentTime / duration) * 100}%` : '0%'
                      }}
                    ></div>
                  </div>
                  <span className="text-xs font-medium text-gray-300 min-w-[40px]">{formatTime(duration)}</span>
                </div>
              </div>

              {/* Right Controls - PhimBro Style */}
      ...
      {/* Episode List Sidebar - PhimBro Style */}
      <div className={`absolute right-0 top-0 bottom-0 w-72 bg-black/90 backdrop-blur-md transition-transform duration-300 ${
        showEpisodeList ? 'translate-x-0' : 'translate-x-full'
      }`}>
        <div className="h-full flex flex-col">
          {/* Sidebar Header */}
          <div className="flex items-center justify-between p-4 border-b border-gray-800">
            <h3 className="text-white text-base font-semibold">Danh sách tập</h3>
            <button
          ...
          <div className="flex-1 overflow-y-auto">
            {episodes.map((server, serverIndex) => (
              <div key={serverIndex} className="p-3">
                <h4 className="text-white font-medium mb-2 text-xs uppercase tracking-wide text-gray-300">
                  {server.server_name}
                </h4>
                <div className="grid grid-cols-4 gap-1.5">
                  {server.server_data.map((episode, episodeIndex) => (
                    <button
                      ...
                      className={`p-2 rounded text-center transition-all duration-200 text-xs font-medium ${
