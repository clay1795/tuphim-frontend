...
import commentApi from '../services/commentApi';
  ...
  useEffect(() => {
    ...
    const loadTopComments = async () => {
      try {
        ...
        // Lấy comments mới nhất từ tất cả các phim
        try {
          console.log('Loading latest comments from all movies...');
          const allComments = [];
          
          // Lấy comments từ tất cả phim trong movieItems
          const moviesToCheck = movieItems.slice(0, 20); // Kiểm tra 20 phim đầu tiên
          
          for (const movie of moviesToCheck) {
            try {
              console.log(`Checking comments for movie: ${movie.slug}`);
              const commentResponse = await commentApi.getTopComments(movie.slug, 5); // Lấy 5 comments mỗi phim
              
              if (commentResponse.success && commentResponse.data.length > 0) {
                console.log(`✅ Found ${commentResponse.data.length} comments for ${movie.name}`);
                ...
                commentResponse.data.forEach(comment => {
                  ...
                  // Hiển thị fullName nếu có, không thì dùng username
                  ...
                  allComments.push({
                    id: comment._id,
                    user: displayName, // Hiển thị fullName hoặc username
                    userInfo: '∞',
                    avatar: avatarUrl,
                    poster: posterUrl,
          ...
          // Sort comments: ưu tiên comments mới nhất, sau đó theo upvotes
          const sortedComments = allComments
            .sort((a, b) => {
              ...
              // Nếu cùng thời gian, sort theo upvotes
              return (b.upvotes || 0) - (a.upvotes || 0);
            })
            .slice(0, 5);
          
          console.log(`📊 Total comments found: ${allComments.length}`);
          console.log(`📊 Top 5 latest comments selected: ${sortedComments.length}`);
          
          if (sortedComments.length > 0) {
            console.log(`✅ SUCCESS: Using ${sortedComments.length} real comments from database`);
            console.log('Latest comments:', sortedComments.map(c => ({ user: c.user, time: c.createdAt, upvotes: c.upvotes })));
            setTopComments(sortedComments);
          } else {
            console.log('⚠️ No real comments found, falling back to mock data');
            // Fallback to mock data
  ...
  const refreshTopComments = async () => {
    try {
      ...
      // Lấy comments mới nhất từ tất cả các phim
      const moviesToCheck = movieData.slice(0, 20); // Kiểm tra 20 phim
      const allComments = [];
      
      for (const movie of moviesToCheck) {
        try {
          const commentResponse = await commentApi.getTopComments(movie.slug, 5);
          if (commentResponse.success && commentResponse.data.length > 0) {
            commentResponse.data.forEach(comment => {
              // Tạo poster URL
              let posterUrl = 'https://via.placeholder.com/100x150/333/fff?text=Movie';
              ...
              const avatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(username)}&background=007bff&color=fff&size=32&bold=true`;
              
              allComments.push({
                id: comment._id,
                user: comment.username, // Hiển thị tên thật của người dùng
                userInfo: '∞',
                avatar: avatarUrl,
                poster: posterUrl,
                comment: comment.content,
      ...
      // Sort comments: ưu tiên comments mới nhất
      const sortedComments = allComments
        .sort((a, b) => {
          const dateA = new Date(a.createdAt);
          const dateB = new Date(b.createdAt);
      ...
      if (sortedComments.length > 0) {
        console.log(`Refreshed: Found ${sortedComments.length} latest comments`);
        console.log('Refreshed comments:', sortedComments.map(c => ({ user: c.user, time: c.createdAt })));
        setTopComments(sortedComments);
      } else {
