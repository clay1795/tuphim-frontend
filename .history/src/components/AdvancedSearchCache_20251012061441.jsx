...
const AdvancedSearchCache = () => {
  ...
  const loadCacheStats = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/movies/cache/stats');
      const result = await response.json();
      if (result.success) {
        setCacheStats(result.data);
        console.log('Cache stats:', result.data);
      }
    } catch (error) {
  ...
  const performSearch = async (page = 1, newFilters = filters, mode = searchMode) => {
    try {
      setLoading(true);
      console.log('=== CACHE SEARCH CALLED ===');
      ...
      console.log('🔍 Cache search URL:', `http://localhost:3001/api/movies/cache/search?${searchParams.toString()}`);
      
      const response = await fetch(`http://localhost:3001/api/movies/cache/search?${searchParams.toString()}`);
      const result = await response.json();
      
      if (!result.success) {
        throw new Error(result.message || 'Cache search failed');
      }
      ...
      if (data.items && Array.isArray(data.items)) {
        ...
        totalPages = data.pagination?.totalPages || 1;
        console.log(`✅ Found ${movieItems.length} movies in cache`);
      } else {
        console.log('❌ No movies found in cache');
      }
      ...
      if (movieItems.length > 0) {
        ...
        console.log(`Found ${movieItems.length} movies (${totalItems} total) from cache`);
      } else {
        setMovies([]);
        setTotalPages(1);
        setTotalResults(0);
      ...
      <div className="bg-gray-800 border-b border-gray-700">
        <div className="container mx-auto px-6 py-8">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-green-300 text-sm">
                    {cacheStats.totalMovies.toLocaleString()} phim trong cache • Cập nhật: {new Date(cacheStats.lastUpdate).toLocaleString()}
                  </p>
                </div>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            <button
              onClick={async () => {
                console.log('Testing cache search API...');
                try {
                  const response = await fetch('http://localhost:3001/api/movies/cache/search?keyword=action&page=1&limit=5');
                  const result = await response.json();
                  console.log('Cache search result:', result);
                } catch (error) {
                  console.error('Cache search error:', error);
                }
              }}
            ...
            <button
              onClick={async () => {
                ...
                try {
                  const response = await fetch('http://localhost:3001/api/movies/cache/stats');
                  const result = await response.json();
                  console.log('Cache stats result:', result);
                  if (result.success) {
                    setCacheStats(result.data);
                  }
                } catch (error) {
            ...
            <button
              onClick={async () => {
                ...
                try {
                  const response = await fetch('http://localhost:3001/api/movies/cache/refresh', { method: 'POST' });
                  const result = await response.json();
            ...
            <button
              onClick={async () => {
                ...
                try {
                  const response = await fetch('http://localhost:3001/api/movies/cache/full-update', { method: 'POST' });
                  const result = await response.json();
                  console.log('Full update result:', result);
                  if (result.success) {
                    setCacheStats(result.data);
                    alert('🚀 Full database update started! This will fetch ALL movies from KKPhim API. Check backend logs for progress.');
                  }
                } catch (error) {
