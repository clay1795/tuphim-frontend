    ...
    try {
      ...
      if (newFilters.type && movieItems.length > 0) {
        ...
        const filteredItems = movieItems.filter(movie => {
          ...
          console.log(`Movie: ${movie.name || movie.movieName} - Type: ${movieType} - Matches: ${matches}`);
          return matches;
        });
        
        console.log(`Filtered ${movieItems.length} movies to ${filteredItems.length} movies of type ${newFilters.type}`);
