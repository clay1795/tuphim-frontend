          ...
          Xem tất cả phim ({searchStats.movies.toLocaleString()})
            ...
            <button
              onClick={async () => {
                ...
                try {
                  // Test all sort options with both directions
                  const sortOptions = ['modified_time', 'created_time', 'year', 'name', 'view'];
            ...
            <button
              onClick={async () => {
                ...
                // Test both directions for each sort option
                const sortOptions = ['modified_time', 'created_time', 'year', 'name', 'view'];
            ...
            {filters.sort && (
              ...
              {filters.sort && (
                <div className="mt-2 text-xs text-cyan-400 bg-cyan-600/10 px-2 py-1 rounded border border-cyan-600/20">
                  📊 Sắp xếp: {sortOptions.find(opt => opt.value === filters.sort)?.label} ({filters.sortType === 'desc' ? 'Mới nhất' : 'Cũ nhất'}) - Đồng bộ với API
                </div>
              )}
            </div>
          )}
