...
const AdvancedSearch = () => {
  ...
  const performSearch = async (page = 1, newFilters = filters, mode = searchMode) => {
    try {
      ...
      const pagesToFetch = hasFilters ? 10 : 50; // Reduced to avoid rate limiting
      ...
      if (newFilters.keyword) {
        console.log('Using keyword search:', newFilters.keyword);
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
          ...
          console.log(`📡 Fetching keyword batch ${batchIndex + 1}/${batches.length}: pages ${batch[0]}-${batch[batch.length-1]}`);
          ...
          batchResults.forEach(({ page, data }) => {
            if (data && data.data && data.data.data && data.data.data.items && Array.isArray(data.data.data.items)) {
              ...
              console.log(`📄 Search page ${page}: ${data.data.data.items.length} movies (Total: ${allMovies.length})`);
            }
          });
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
          const batch = batches[batchIndex];
          console.log(`📡 Fetching category batch ${batchIndex + 1}/${batches.length}: pages ${batch[0]}-${batch[batch.length-1]}`);
          ...
          const batchPromises = batch.map(async (currentPage) => {
            try {
              ...
              console.error(`Error fetching category page ${currentPage}:`, error);
              return { page: currentPage, data: null };
            }
          });
          ...
          batchResults.forEach(({ page, data }) => {
            if (data && data.data && data.data.data && data.data.data.items && Array.isArray(data.data.data.items)) {
              ...
              totalPagesResult = data.data.data.params?.pagination?.totalPages || 1;
              totalItemsResult = data.data.data.params?.pagination?.totalItems || allMovies.length;
              console.log(`📄 Category page ${page}: ${data.data.data.items.length} movies (Total: ${allMovies.length})`);
            }
          });
        ...
        console.log('Using new movies (no filters)');
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
          const batch = batches[batchIndex];
          console.log(`📡 Fetching new movies batch ${batchIndex + 1}/${batches.length}: pages ${batch[0]}-${batch[batch.length-1]}`);
  ...
  const handleFilterChange = (key, value) => {
    ...
    console.log('🔍 Triggering search with new filters:', newFilters);
          ...
          <form onSubmit={handleSearch} className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div>
                ...
                >
                  <option value="">{loadingFilters ? "Đang tải..." : "Tất cả thể loại"}</option>
              ...
              <div>
                ...
                <select
                  ...
                  disabled={loadingFilters}
                >
                  <option value="">{loadingFilters ? "Đang tải..." : "Tất cả quốc gia"}</option>
