      ...
      <div className="container mx-auto px-3 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 md:mb-8">
          ...
          {showViewAll && linkTo && (
            <Link 
              to={linkTo}
              className="text-red-400 hover:text-red-300 transition-colors font-medium flex items-center space-x-1 sm:space-x-2 bg-red-400/10 hover:bg-red-400/20 px-3 sm:px-4 py-2 rounded-lg border border-red-400/30 self-start sm:self-auto"
            >
              <span className="text-sm sm:text-base">Xem thêm</span>
        ...
        {movies && movies.length > 0 ? (
          <div className="relative">
            ...
            <div className="flex overflow-x-auto scrollbar-hide space-x-3 sm:space-x-4 md:space-x-6 pb-4 mobile-scroll" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              {movies.map((movie, index) => (
                <div key={movie._id} className="flex-shrink-0 w-36 sm:w-40 md:w-48">
                  <SeparatedMovieCard 
                    movie={movie}
                    index={index}
                  />
