...
const SyncManager = () => {
  ...
  const runIncrementalSync = async () => {
    setLoading(true);
    try {
      const response = await fetch('getApiUrl()/sync/incremental', {
        method: 'POST'
      });
      const data = await response.json();
      if (data.success) {
  ...
  const forceHourlySync = async () => {
    setLoading(true);
    try {
      const response = await fetch('getApiUrl()/sync/force-hourly', {
        method: 'POST'
      });
      const data = await response.json();
      if (data.success) {
  ...
  const toggleScheduler = async (action) => {
    setLoading(true);
    try {
      const response = await fetch(`getApiUrl()/sync/${action}-scheduler`, {
        method: 'POST'
      });
      const data = await response.json();
      if (data.success) {
