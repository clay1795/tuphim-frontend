...
import simpleSearchApi from '../services/simpleSearchApi';
...
const AdvancedSearch = () => {
  ...
  const [loadingProgress] = useState({ current: 0, total: 0, percentage: 0 });
  ...
  const performSearch = async (page = 1, newFilters = filters) => {
    try {
      ...
      const data = await simpleSearchApi.searchMovies(newFilters.keyword, searchOptions);
  ...
  const showAllMoviesNow = async () => {
    try {
      ...
      const data = await simpleSearchApi.getAllMovies({
        limit: 50,
        page: 1,
  ...
  const clearCache = async () => {
    try {
      const success = await simpleSearchApi.clearCache();
      if (success) {
  ...
  useEffect(() => {
    ...
    const initializeSearch = async () => {
      try {
        ...
        const stats = await simpleSearchApi.initialize();
        setSearchStats(stats);
        setFullDatabaseLoading(false);
        
        // Update cache status
        const cacheStatus = await simpleSearchApi.getCacheStatus();
            ...
            <div className="max-w-2xl mx-auto">
              <div className="bg-green-600/20 border border-green-600/30 rounded-lg p-4">
                <p className="text-green-400 text-sm">
                  ⚡ <strong>Tìm kiếm tức thì:</strong> Tìm trong {searchStats.movies.toLocaleString()} phim từ database
                </p>
              </div>
            </div>
