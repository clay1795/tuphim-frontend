...
import commentApi from '../services/commentApi';
  ...
  useEffect(() => {
    const loadTopComments = async () => {
      try {
        ...
        // Lấy top comments từ các phim này
        const allComments = [];
        
        for (const movie of movieItems.slice(0, 5)) {
          try {
            const commentResponse = await commentApi.getTopComments(movie.slug, 1);
        ...
        // Nếu không có comments thật, tạo mock data dựa trên phim
        if (allComments.length === 0) {
          console.log('No real comments found, using mock data');
          ...
          setTopComments(allComments);
        }
        
        console.log('Top comments loaded:', allComments.length, 'real comments');
  ...
  const refreshTopComments = async () => {
    try {
      ...
      for (const movie of movieData.slice(0, 5)) {
        try {
          const commentResponse = await commentApi.getTopComments(movie.slug, 1);
          if (commentResponse.success && commentResponse.data.length > 0) {
            const comment = commentResponse.data[0];
      ...
      // Nếu không có comments thật, tạo mock data
      if (allComments.length === 0) {
