...
const AdvancedSearch = () => {
  ...
  const performSearch = async (page = 1, newFilters = filters, mode = searchMode) => {
    try {
      ...
      if (data.items && data.items.length > 0) {
        ...
        setSearchMode(data.instant ? 'instant' : data.extended ? 'extended' : 'full');
  ...
  useEffect(() => {
    ...
    initializeSearch();
  }, [filters]); // Add filters as dependency to avoid warning
          ...
          <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-3 bg-green-600/20 border border-green-600/30 rounded-lg">
              <p className="text-green-400 text-sm font-medium">
                ⚡ Tìm kiếm tức thì: {searchStats.movies} phim
              </p>
            </div>
