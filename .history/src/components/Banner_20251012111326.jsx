...
const Banner = ({ onFeaturedMovieChange }) => {
  ...
  const loadFeaturedMovie = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}/mongo-movies/new?page=1&limit=24`);
      ...
      if (data.success && data.data && data.data.items && data.data.items.length > 0) {
        ...
        if (!movie.content && movie.slug) {
          try {
            ...
            const response = await fetch(`${API_CONFIG.BASE_URL}/mongo-movies/detail/${movie.slug}`);
