              ...
              {comments.map((comment) => (
                <div key={comment._id} className="bg-gray-800 rounded-lg p-4">
                  <div className="flex items-start space-x-3">
                    <img
                      src={getAvatarUrl(comment.avatar, (comment.fullName && comment.fullName.trim()) || comment.username)}
                      alt={(comment.fullName && comment.fullName.trim()) || comment.username}
                      className="w-10 h-10 rounded-full flex-shrink-0"
                      onError={(e) => {
                        const displayName = (comment.fullName && comment.fullName.trim()) || comment.username;
