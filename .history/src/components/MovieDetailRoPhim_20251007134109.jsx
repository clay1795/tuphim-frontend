...
import { getMovieVideos, getYouTubeTrailerUrl } from "../services/tmdbApi";
...
// Helper function to get trailer URL from TMDB
const getTrailerUrl = (movie) => {
  if (!movie || !movie.tmdb || !movie.tmdb.id) return null;
  
  const tmdbId = movie.tmdb.id;
  const type = movie.tmdb.type || 'movie';
  
  // Tạo trailer URL từ TMDB ID
  // Sử dụng TMDB API để lấy trailer thật
  return `https://www.themoviedb.org/${type}/${tmdbId}/videos`;
};

// Helper function to get fallback YouTube trailer embed URL
const getFallbackTrailerUrl = (movie) => {
  if (!movie || !movie.tmdb || !movie.tmdb.id) return null;
  ...
  const trailerVideos = {
    '223326': 'https://www.youtube.com/embed/example1', // Gióng Tô 2025
    '272484': 'https://www.youtube.com/embed/example2', // Nhất Tiêu Tuy Ca
...
const MovieDetailRoPhim = () => {
  ...
  useEffect(() => {
    const loadMovieDetail = async () => {
      try {
        ...
        // Load trailer data if movie has TMDB ID
        if (movieData && movieData.tmdb && movieData.tmdb.id) {
          loadTrailerData(movieData.tmdb.id, movieData.tmdb.type);
        }
  ...
  // Function to load trailer data from TMDB
  const loadTrailerData = async (tmdbId, type = 'movie') => {
    try {
      setLoadingTrailer(true);
      console.log('Loading trailer data for TMDB ID:', tmdbId, 'Type:', type);
      
      const videosData = await getMovieVideos(tmdbId, type);
      if (videosData) {
        setTrailerData(videosData);
        console.log('Trailer data loaded:', videosData);
      }
    } catch (error) {
  ...
  const handlePlayTrailer = () => {
    // Sử dụng trailer data thật từ TMDB nếu có
    if (trailerData) {
      const trailer = getYouTubeTrailerUrl(trailerData);
      if (trailer) {
    ...
    // Fallback to mock trailer
    const fallbackTrailer = getFallbackTrailerUrl(movie);
    if (fallbackTrailer) {
      setTrailerUrl(fallbackTrailer);
      setShowTrailer(true);
    }
  };
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="relative min-h-[80vh] bg-gray-900">
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${getOptimizedImageUrl(movie.poster_url || movie.thumb_url)})`
          }}
        />
                ...
                <div>
                  ...
                  <div className="space-y-8">
                    ...
                    <div>
                      ...
                      {/* Trailer Section */}
                      {(movie?.tmdb?.id || trailerData) && (
                        <div className="mb-6">
                          <div className="bg-gray-800 rounded-lg p-6">
                            <div className="flex items-center justify-between mb-4">
                              <h4 className="text-lg font-semibold text-white">🎬 Trailer Chính Thức</h4>
                                ...
                                {loadingTrailer ? (
                                  ...
                                  <>
                                    <button
                                      ...
                                      className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-4 py-2 rounded-lg font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center gap-2"
                                    >
                              ...
                              <div className="aspect-video bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center">
                                {loadingTrailer ? (
                                  <div className="text-center">
                                    ...
                                    <p className="text-white font-semibold">Đang tải trailer...</p>
                                    <p className="text-gray-400 text-sm mt-1">Lấy dữ liệu từ TMDB</p>
                                  </div>
                                ) : (
                                  <div className="text-center">
                                    <div className="w-20 h-20 bg-red-600 rounded-full flex items-center justify-center mx-auto mb-4 cursor-pointer hover:bg-red-700 transition-colors" onClick={handlePlayTrailer}>
                                      <svg className="w-8 h-8 text-white ml-1" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M8 5v14l11-7z"/>
                                      </svg>
                                    </div>
                                    <p className="text-white font-semibold">Nhấn để xem trailer</p>
                                    <p className="text-gray-400 text-sm mt-1">
                                      {trailerData ? 'Trailer chính thức từ TMDB' : 'Trailer từ TMDB'}
                                    </p>
                                    {trailerData && (
                                      <p className="text-gray-500 text-xs mt-1">
                                        {trailerData.results?.length || 0} video(s) có sẵn
                                      </p>
                                    )}
                                  </div>
                                )}
                      ...
                      <div className="bg-gray-800 rounded-lg p-6">
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-gray-400 font-semibold">Chưa có video khác</p>
                          <p className="text-gray-500 text-sm mt-1">Không có clip hậu trường hay video bổ sung</p>
                        </div>
                      </div>
                    </div>
