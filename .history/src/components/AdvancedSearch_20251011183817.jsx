...
const AdvancedSearch = () => {
  ...
  const performSearch = async (page = 1, newFilters = filters, mode = searchMode) => {
    try {
      ...
      // Fetch multiple pages but not too many to avoid crashes
      const hasFilters = newFilters.keyword || newFilters.category || newFilters.country || newFilters.year || newFilters.type;
      const pagesToFetch = hasFilters ? 10 : 20; // Reduced to prevent crashes and timeouts
      console.log(`📄 Fetching ${pagesToFetch} page(s) - Has filters: ${hasFilters} (Database: 24,673 movies, Target: ${pagesToFetch * 24} movies)`);
