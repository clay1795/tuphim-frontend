...
const AdvancedSearch = () => {
  ...
  const performSearch = async (page = 1, newFilters = filters) => {
    try {
      ...
      // Sử dụng full database search (đã có cache)
      const data = await instantSearchApi.getAllMovies(searchOptions);
            ...
            <div className="space-y-4 max-w-2xl mx-auto">
              <div className="bg-green-600/20 border border-green-600/30 rounded-lg p-4">
                <p className="text-green-400 text-sm">
                  ⚡ <strong>Tìm kiếm tức thì:</strong> Tìm trong {searchStats.movies.toLocaleString()} phim đã tải sẵn - Nhanh nhất
                </p>
              </div>
              ...
              <div className="bg-purple-600/20 border border-purple-600/30 rounded-lg p-4">
                <p className="text-purple-400 text-sm">
                  🌐 <strong>Tìm kiếm đầy đủ:</strong> {searchStats.movies >= 10000 ? `Tìm trong ${searchStats.movies.toLocaleString()} phim - Kết quả đầy đủ nhất` : 'Đang tự động load toàn bộ database...'}
                </p>
              </div>
            </div>
