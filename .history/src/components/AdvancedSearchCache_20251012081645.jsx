...
const AdvancedSearchCache = () => {
  ...
  const [searchMode, setSearchMode] = useState('mongodb'); // cache, api, mongodb
  ...
  const handlePageChange = (page) => {
    setCurrentPage(page);
    performSearch(page, filters, 'mongodb');
  };
  ...
  // Trigger search when filters change (except for URL params changes)
  useEffect(() => {
    console.log('=== FILTERS CHANGED, TRIGGERING SEARCH ===');
    console.log('Current filters:', filters);
    performSearch(1, filters, 'mongodb');
  }, [filters.sort, filters.sortType, filters.type, filters.category, filters.country, filters.year]);
