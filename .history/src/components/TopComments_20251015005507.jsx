  ...
  useEffect(() => {
    ...
    const loadTopComments = async () => {
      try {
        ...
        // Lấy comments từ database với logic đơn giản
        try {
          console.log('Loading comments with simplified logic...');
          const allComments = [];
          
          // Test với phim cụ thể có comments
          const testMovies = ['tieu-diet-ca-map', 'co-gai-ca-map'];
          
          for (const slug of testMovies) {
            try {
              console.log(`Fetching comments for movie: ${slug}`);
              const commentResponse = await commentApi.getTopComments(slug, 3);
              ...
              if (commentResponse.success && commentResponse.data.length > 0) {
                ...
                commentResponse.data.forEach(comment => {
                  ...
                  const movie = movieItems.find(m => m.slug === slug);
                  console.log(`Movie found for ${slug}:`, movie ? movie.name : 'Not found');
                  
                  // Tạo poster URL đơn giản
                  let posterUrl = 'https://via.placeholder.com/100x150/333/fff?text=Movie';
                  if (movie && movie.thumb_url) {
                    posterUrl = movie.thumb_url;
                  } else if (movie && movie.poster_url) {
                    posterUrl = movie.poster_url;
                  } else if (movie && movie.thumbnail) {
                    posterUrl = movie.thumbnail;
                  }
                  console.log(`Poster URL for ${movie?.name || slug}:`, posterUrl);
                  
                  // Tạo avatar URL đơn giản
                ...
                console.log(`❌ No comments found for ${slug}`);
              }
            } catch (commentError) {
              console.log(`❌ Error fetching comments for ${slug}:`, commentError);
            }
          }
