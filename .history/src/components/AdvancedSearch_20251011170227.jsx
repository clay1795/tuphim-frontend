...
const AdvancedSearch = () => {
  ...
  const [loadingProgress] = useState({ percentage: 0, current: 0, total: 0 });
  ...
  const performSearch = useCallback(async (page = 1, newFilters = filters, mode = searchMode) => {
    try {
      ...
      const pagesToFetch = hasFilters ? 3 : 5; // Reduced pages to avoid empty results
      ...
      console.log('🔍 Processing response data structure:', {
        ...
        dataItemsLength: data.items?.length || 0,
        dataDataItemsLength: data.data?.items?.length || 0,
