  ...
  const performSearch = async (page = 1, newFilters = filters) => {
    try {
      ...
      if (newFilters.keyword) {
        // Kiểm tra instant search trước
        const instantKeywords = ['vietnam', 'viet', 'korea', 'han', 'china', 'trung', 'japan', 'nhat', 'action', 'romance', 'comedy', 'horror', 'drama'];
        const isInstantKeyword = instantKeywords.includes(newFilters.keyword.toLowerCase());
        
        if (isInstantKeyword && page === 1) {
          // Sử dụng instant search cho từ khóa phổ biến
          console.log(`Using instant search for: ${newFilters.keyword}`);
          data = await simpleMovieApi.instantSearch(newFilters.keyword);
        } else {
