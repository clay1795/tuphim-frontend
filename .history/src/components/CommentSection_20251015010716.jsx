  ...
  const handlePostComment = async (e) => {
    ...
    try {
      ...
      const response = await commentApi.postComment({
        ...
        fullName: user?.fullName || username.trim() // Thêm fullName từ user hoặc dùng username
      });
              ...
              {comments.map((comment) => (
                <div key={comment._id} className="bg-gray-800 rounded-lg p-4">
                  <div className="flex items-start space-x-3">
                    <img
                      src={getAvatarUrl(comment.avatar, comment.username)}
                      alt={comment.username}
                      className="w-10 h-10 rounded-full flex-shrink-0"
                      onError={(e) => {
                        e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(comment.username.charAt(0))}&background=4B5563&color=fff&size=40`;
                      }}
                    />
