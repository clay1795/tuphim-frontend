...
import AudioTypeSelector from './AudioTypeSelector.jsx';
...
const MovieDetailRoPhim = () => {
  ...
  const { updatePreferences, getPreferencesForMovie } = useVideoPlayer();
  ...
  const [selectedAudioType, setSelectedAudioType] = useState("subtitle");
  ...
  const handlePlayEpisode = async (episode, serverIndex, episodeIndex) => {
    if (user) {
    ...
    // Save user preferences before navigating
    updatePreferences(selectedAudioType, serverIndex, episodeIndex, slug, episodes);

    // Navigate to video player page instead of showing modal
    ...
    const loadMovieDetail = async () => {
      try {
        ...
        if (episodesData && episodesData.length > 0) {
          ...
          const userPrefs = getPreferencesForMovie(slug);
          if (userPrefs.serverIndex !== undefined && userPrefs.serverIndex < episodesData.length) {
            setSelectedServer(episodesData[userPrefs.serverIndex]);
            setSelectedSeason(episodesData[userPrefs.serverIndex].server_name);
            if (episodesData[userPrefs.serverIndex].server_data && 
                userPrefs.episodeIndex < episodesData[userPrefs.serverIndex].server_data.length) {
              setSelectedEpisode(episodesData[userPrefs.serverIndex].server_data[userPrefs.episodeIndex]);
            } else {
              setSelectedEpisode(episodesData[userPrefs.serverIndex].server_data[0]);
            }
            setSelectedAudioType(userPrefs.audioType);
          } else {
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="relative min-h-[80vh] bg-gray-900">
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${getOptimizedImageUrl(movie.banner_url || movie.thumb_url)})`
          }}
        />
        ...
        {/* Content */}
        <div className="relative z-10 pt-32 sm:pt-40 md:pt-48 lg:pt-56 pb-4 sm:pb-6 md:pb-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 items-start">
                ...
                <div className="lg:col-span-1 flex flex-col items-center lg:items-start">
                  <div className="relative group">
                    <div className="absolute -inset-2 sm:-inset-4 bg-gradient-to-r from-yellow-500/20 to-red-500/20 rounded-xl sm:rounded-2xl blur-lg sm:blur-xl group-hover:blur-xl sm:group-hover:blur-2xl transition-all duration-500"></div>
                    <img
                      src={getOptimizedImageUrl(movie.poster_url)}
                      alt={movie.name}
                      className="relative w-48 sm:w-56 md:w-64 h-[18rem] sm:h-[21rem] md:h-[24rem] object-cover object-center rounded-lg sm:rounded-xl shadow-xl sm:shadow-2xl group-hover:scale-102 sm:group-hover:scale-105 transition-all duration-500 border-2 border-white/10"
                      style={{
                  ...
                  <div className="flex flex-col items-center gap-3 mt-4 w-64">
                    ...
                    {episodes && episodes.length > 0 && (
                      <button
                        onClick={() => {
                          ...
                          if (episodes && episodes.length > 0 && episodes[0].server_data && episodes[0].server_data.length > 0) {
                            // Save user preferences before navigating
                            updatePreferences(selectedAudioType, 0, 0, slug, episodes);
                            navigate(`/xem-phim/${slug}?ver=1&ss=1&ep=1`);
                          }
                        }}
                    ...
                    <div className="flex items-center justify-center gap-6 w-full">
                      ...
                      >
                        ...
                        <span className="text-white text-xs font-medium">Xem sau</span>
                      </button>
                    </div>
            ...
            <div className="min-h-[400px]">
              {activeTab === 'episodes' && (
                <div className="space-y-6">
                  ...
                  {episodes.length > 1 && (
                    <div className="mb-6">
                      <h4 className="text-[#FFC94A] font-semibold mb-3">Chọn server phát:</h4>
                      <div className="flex flex-wrap gap-2">
                        {episodes.map((server, index) => {
                          // Clean server name - remove location info like "#Hà Nội"
                          let cleanServerName = server.server_name || `Server ${index + 1}`;
                          
                          // Remove location patterns like "#Hà Nội (Vietsub)" -> "Vietsub"
                          cleanServerName = cleanServerName
                            .replace(/#[^(]*\(([^)]+)\)/g, '$1') // Extract content inside parentheses after #
                            .replace(/#[^)]*\)/g, '') // Remove remaining #location)
                          ...
                          if (!cleanServerName) {
                            cleanServerName = server.server_name || `Server ${index + 1}`;
                          }
                  ...
                  {selectedServer && selectedServer.server_data && (
                    <div>
                      <h4 className="text-[#FFC94A] font-semibold mb-3">
                        Danh sách tập:
                      </h4>
                      <EpisodeSelector
                        episodes={selectedServer.server_data}
                        currentEpisode={selectedEpisode ? selectedServer.server_data.indexOf(selectedEpisode) : 0}
                        currentServer={episodes.indexOf(selectedServer)}
                        onEpisodeChange={(serverIndex, episodeIndex) => {
                          const episode = episodes[serverIndex]?.server_data?.[episodeIndex];
                          if (episode) {
                            handlePlayEpisode(episode, serverIndex, episodeIndex);
                          }
                        }}
                        maxVisible={12}
                  ...
                  {/* Tuỳ chọn âm thanh/phụ đề */}
                  <div>
                    <h4 className="text-[#FFC94A] font-semibold mb-3">Tuỳ chọn âm thanh/phụ đề:</h4>
                    <AudioTypeSelector
                      audioType={selectedAudioType}
                      onAudioChange={setSelectedAudioType}
                      hasDubVersion={selectedServer?.server_data?.some(ep => ep.link_dub)}
                      showInfo={true}
                    />
                    <p className="text-gray-400 text-xs mt-2">
                      Tùy chọn này sẽ được áp dụng khi bạn bấm &quot;Xem Ngay&quot; hoặc chọn tập phim
                    </p>
                  </div>
      ...
      {/* Video Player Modal removed - now using separate VideoPlayer page */}
    </div>
  );
};
