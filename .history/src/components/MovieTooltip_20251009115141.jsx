...
const MovieTooltip = ({ movie, children }) => {
  ...
  const formatEpisodeStatus = (episodeCurrent, episodeTotal, status) => {
    ...
    const currentNum = episodeCurrent.toString().replace(/[^\d]/g, '');
    ...
    const isCompleted = status === 'completed' || 
                       ...
                       episodeCurrent.toString().toLowerCase().includes('hoàn tất') ||
                       episodeCurrent.toString().toLowerCase().includes('complete') ||
                       episodeCurrent.toString().toLowerCase().includes('finished');
    
    console.log('🎬 Episode Status Debug:', {
      episodeCurrent,
      episodeTotal,
      status,
      currentNum,
              ...
              {enhancedMovie.episode_current && (
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full border ${
                    (() => {
                      // Use same logic as formatEpisodeStatus for consistency
                      const currentNum = enhancedMovie.episode_current.toString().replace(/[^\d]/g, '');
                      const totalNum = enhancedMovie.episode_total ? enhancedMovie.episode_total.toString().replace(/[^\d]/g, '') : null;
                      
                      const isCompleted = enhancedMovie.status === 'completed' || 
                                         enhancedMovie.status === 'hoàn thành' || 
                                         enhancedMovie.status === 'finished' ||
                                         enhancedMovie.status === 'ended' ||
                                         enhancedMovie.status === 'complete' ||
                                         (totalNum && parseInt(currentNum) >= parseInt(totalNum)) ||
                                         enhancedMovie.episode_current.toString().toLowerCase().includes('hoàn tất') ||
                                         enhancedMovie.episode_current.toString().toLowerCase().includes('complete') ||
                                         enhancedMovie.episode_current.toString().toLowerCase().includes('finished');
