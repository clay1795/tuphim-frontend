...
const AdvancedSearch = () => {
  ...
  const sortOptions = [
    { value: 'modified_time', label: 'Ngày cập nhật', defaultSort: 'desc' },
    { value: 'year', label: 'Ngày phát hành', defaultSort: 'desc' },
    { value: 'view', label: 'Điểm đánh giá', defaultSort: 'desc' }
  ];
  ...
  const performSearch = useCallback(async (page = 1, newFilters = filters, mode = searchMode) => {
    try {
      ...
      const pagesToFetch = hasFilters ? 3 : 5; // Fetch fewer pages to avoid empty results
      console.log(`📄 Fetching ${pagesToFetch} page(s) - Has filters: ${hasFilters}`);
      ...
      try {
        ...
        if (newFilters.keyword) {
          ...
          // If no movies found in type search, fallback to new movies and filter by type
          if (allMovies.length === 0) {
            console.log(`🔄 No movies found for type "${newFilters.type}", falling back to new movies with type filter`);
          ...
          // Fetch multiple pages for new movies
          let allMovies = [];
          let totalPages = 1;
          let totalItems = 0;
          ...
          for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
            ...
            if (result.data && result.data.data && result.data.data.items && Array.isArray(result.data.data.items)) {
              ...
              totalPages = result.data.data.params?.pagination?.totalPages || 1;
              totalItems = result.data.data.params?.pagination?.totalItems || allMovies.length;
              console.log(`📄 New movies page ${currentPage}: ${result.data.data.items.length} movies (Total: ${allMovies.length})`);
              ...
              console.log(`❌ New movies page ${currentPage}: Invalid response structure`);
              console.log('Response structure:', {
                hasData: !!result.data,
                hasDataData: !!result.data?.data,
                hasItems: !!result.data?.items,
                hasDataItems: !!result.data?.data?.items,
                isArray: Array.isArray(result.data?.items || result.data?.data?.items),
                itemsLength: (result.data?.items || result.data?.data?.items)?.length || 0
              });
            }
          }
      ...
      <div className="bg-gray-800 border-b border-gray-700">
        <div className="container mx-auto px-6 py-8">
              ...
              <div>
                ...
                >
                  {sortOptions.map(option => (
                    <option key={`${option.value}-${option.defaultSort}`} value={`${option.value}-${option.defaultSort}`}>
                      {option.label}
                    </option>
                  ))}
                </select>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            <button
              onClick={() => {
                console.log('Test Instant Search - Using backend API');
            ...
            <button
              onClick={() => {
                console.log('Test Full Database - Using backend API');
            ...
            <button
              onClick={() => {
                console.log('Load Full Database - Using backend API');
                console.log('Triggering search with current filters...');
                performSearch(1, filters, 'full');
              }}
