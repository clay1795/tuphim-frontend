    ...
    try {
      ...
      try {
        if (newFilters.keyword) {
          ...
          const response = await fetch(`http://localhost:3001/api/movies/search?keyword=${encodeURIComponent(newFilters.keyword)}&page=${page}&limit=20`);
          const result = await response.json();
          data = result.data;
        } else if (newFilters.genres.length > 0) {
          console.log('Searching by category:', newFilters.genres[0]);
          const response = await fetch(`http://localhost:3001/api/movies/category/${newFilters.genres[0]}?page=${page}&limit=20`);
          const result = await response.json();
          data = result.data;
        } else if (newFilters.countries.length > 0) {
          console.log('Searching by country:', newFilters.countries[0]);
          const response = await fetch(`http://localhost:3001/api/movies/country/${newFilters.countries[0]}?page=${page}&limit=20`);
          const result = await response.json();
          data = result.data;
        } else if (newFilters.years.length > 0) {
          console.log('Searching by year:', newFilters.years[0]);
          const response = await fetch(`http://localhost:3001/api/movies/year/${newFilters.years[0]}?page=${page}&limit=20`);
          const result = await response.json();
          data = result.data;
        } else if (newFilters.types.length > 0) {
          console.log('Searching by type:', newFilters.types[0]);
          data = await simpleMovieApi.getMoviesByType(newFilters.types[0], searchParams);
        } else {
          console.log('Loading new movies');
          data = await simpleMovieApi.getNewMovies(page);
        }
      } catch (apiError) {
      ...
      if (movieData.length === 0 && !newFilters.keyword && newFilters.genres.length === 0 && newFilters.countries.length === 0 && newFilters.years.length === 0 && newFilters.types.length === 0) {
        ...
        try {
          const fallbackData = await simpleMovieApi.getNewMovies(page);
