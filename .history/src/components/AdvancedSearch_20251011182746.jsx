...
const AdvancedSearch = () => {
  ...
  const performSearch = async (page = 1, newFilters = filters, mode = searchMode) => {
    try {
      ...
      const pagesToFetch = hasFilters ? 20 : 50; // Fetch many more pages to get more movies from 24,673 database
      console.log(`📄 Fetching ${pagesToFetch} page(s) - Has filters: ${hasFilters} (Database: 24,673 movies)`);
        ...
        if (newFilters.keyword) {
          ...
          if (allMovies.length === 0) {
            ...
            for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
              ...
              const response = await fetch(`http://localhost:3001/api/movies/new?page=${currentPage}&limit=100&version=v3`);
              const result = await response.json();
