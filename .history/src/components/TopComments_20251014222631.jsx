            ...
            try {
              ...
              if (commentResponse.success && commentResponse.data.length > 0) {
                ...
                commentResponse.data.forEach(comment => {
                  ...
                  if (movie) {
                    posterUrl = getOptimizedImageUrl(movie);
                    console.log(`Poster URL for ${movie.name}:`, posterUrl);
                  }
                  
                  // Tạo avatar URL
                  let avatarUrl = `https://via.placeholder.com/32x32/007bff/fff?text=${(comment.username?.charAt(0) || 'U').toUpperCase()}`;
                  if (movie) {
                    avatarUrl = getRealAvatarFromApi(movie, 0);
                    console.log(`Avatar URL for ${movie.name}:`, avatarUrl);
                  }
