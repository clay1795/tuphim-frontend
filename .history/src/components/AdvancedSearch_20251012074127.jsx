      ...
      try {
        console.log('Using instant search API with safe error handling');
         ...
         if (newFilters.keyword) {
           ...
           const response = await fetch(`http://localhost:3001/api/mongo-movies/search?${searchParams.toString()}`);
          ...
          data = result.data;
           ...
           console.log('Using new movies API');
           const response = await fetch(`http://localhost:3001/api/movies/new?page=${page}&version=v3`);
           const result = await response.json();
           data = result.data;
         }
          ...
          <form onSubmit={handleSearch} className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <div>
                ...
                <div className="text-xs text-gray-400 mb-1">Current: {filters.sort}-{filters.sortType}</div>
                <button 
                  onClick={() => {
                    console.log('🔍 Current filters state:', filters);
                    console.log('🔍 Current movies count:', movies.length);
                  }}
                  className="text-xs bg-blue-600 px-2 py-1 rounded mb-1 mr-1"
                >
                  Debug State
                </button>
                <button 
                  onClick={async () => {
                    console.log('🧪 Testing year desc sort...');
                    try {
                      const response = await fetch('http://localhost:3001/api/mongo-movies/search?keyword=&page=1&limit=3&sort=year&sortType=desc');
                      const result = await response.json();
                      console.log('🧪 Test result:', result.data.items[0]);
                    } catch (error) {
                ...
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    console.log('🎯 Parsed sort values:', { sort, sortType });
                    ...
                    // Trigger search with new filters immediately
                    try {
                      setLoading(true);
                      console.log('Sort changed to:', sort, sortType);
                      ...
                      if (data.items && Array.isArray(data.items)) {
                        ...
                        console.log(`✅ Sort applied: ${sort} ${sortType}, found ${data.items.length} movies`);
                      }
                    } catch (error) {
