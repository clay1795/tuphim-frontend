...
const AdvancedSearch = () => {
  ...
  const loadFullDatabase = async () => {
    try {
      ...
      // Perform search with full database
  ...
  useEffect(() => {
    // Initialize instant search API
    const initializeSearch = async () => {
      try {
        console.log('Initializing instant search...');
        const stats = await instantSearchApi.initialize();
        setSearchStats(stats);
        console.log('Instant search initialized:', stats);
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="bg-gray-800 border-b border-gray-700">
        <div className="container mx-auto px-6 py-8">
          ...
          <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-3 bg-green-600/20 border border-green-600/30 rounded-lg">
              <p className="text-green-400 text-sm font-medium">
                ⚡ Tìm kiếm tức thì: {searchStats.movies.toLocaleString()} phim
              </p>
            </div>
            ...
            <div className="p-3 bg-purple-600/20 border border-purple-600/30 rounded-lg">
              <p className="text-purple-400 text-sm font-medium">
                🔍 Chế độ: {searchMode === 'instant' ? 'Tức thì' : searchMode === 'extended' ? 'Mở rộng' : 'Đầy đủ'}
              </p>
            </div>
          </div>

          {/* Full Database Loading Controls */}
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  <h3 className="text-yellow-400 font-medium mb-2">
                    🚀 Load Toàn Bộ Database ({searchStats.movies.toLocaleString()} phim hiện tại)
                  </h3>
                  <p className="text-yellow-300 text-sm">
                    Load toàn bộ 20,000+ phim để tìm kiếm tức thì trong toàn bộ database
                  </p>
                </div>
          ...
          {fullDatabaseLoading && (
            <div className="mb-4 p-4 bg-blue-600/20 border border-blue-600/30 rounded-lg">
              ...
              <p className="text-blue-300 text-sm">
                Đang tải {loadingProgress.current} trang... Ước tính còn {Math.ceil((loadingProgress.total - loadingProgress.current) * 0.5)} giây
              </p>
            </div>
          )}
          ...
          {/* Results Info & Search Mode Controls */}
          <div className="mt-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              >
                🌐 Đầy đủ {searchStats.movies >= 10000 ? `(${searchStats.movies.toLocaleString()})` : '(Chưa load)'}
              </button>
              {searchStats.movies >= 10000 && (
                <button
                  type="button"
        ...
        {loading ? (
            ...
            <div className="bg-gray-800 rounded-lg p-6 text-center">
              <div className="flex items-center justify-center space-x-2">
                ...
                <span className="text-white font-medium">
                  {searchMode === 'instant' ? 'Đang tìm kiếm tức thì...' : 
                   searchMode === 'extended' ? 'Đang tìm kiếm mở rộng...' : 
                   'Đang tìm kiếm đầy đủ...'}
                </span>
              </div>
              <p className="text-gray-400 text-sm mt-2">
                {searchMode === 'instant' ? 'Tìm trong 200 phim đã tải sẵn' :
                 searchMode === 'extended' ? 'Tìm trong 400 phim từ database' :
          ...
          <div className="text-center py-16">
            ...
            <div className="text-gray-500 text-sm mb-6">
              Thử thay đổi từ khóa tìm kiếm hoặc chuyển sang chế độ tìm kiếm khác
            </div>
            ...
            <div className="space-y-4 max-w-2xl mx-auto">
              <div className="bg-green-600/20 border border-green-600/30 rounded-lg p-4">
                <p className="text-green-400 text-sm">
                  ⚡ <strong>Tìm kiếm tức thì:</strong> Tìm trong 200 phim đã tải sẵn - Nhanh nhất
                </p>
              </div>
              <div className="bg-blue-600/20 border border-blue-600/30 rounded-lg p-4">
                <p className="text-blue-400 text-sm">
                  🔍 <strong>Tìm kiếm mở rộng:</strong> Tìm trong 400 phim từ database - Cân bằng tốc độ và kết quả
                </p>
              </div>
              <div className="bg-purple-600/20 border border-purple-600/30 rounded-lg p-4">
                <p className="text-purple-400 text-sm">
                  🌐 <strong>Tìm kiếm đầy đủ:</strong> Tìm trong toàn bộ database - Kết quả đầy đủ nhất
                </p>
              </div>
            </div>
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-4">
              <button
                onClick={() => {
                  setFilters({ ...filters, keyword: '', category: '', country: '', year: '', type: '' });
                  performSearch(1, { ...filters, keyword: '', category: '', country: '', year: '', type: '' });
                }}
