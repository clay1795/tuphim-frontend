  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      if (authMode === 'login') {
        ...
        // Validate input data
        if (!authForm.displayName || !authForm.email || !authForm.password) {
        ...
        if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(authForm.password)) {
          setAuthError('Mật khẩu phải có chữ hoa, chữ thường và số');
          setAuthLoading(false);
          return;
        }

        // Validate display name
        if (authForm.displayName.length < 2 || authForm.displayName.length > 50) {
          setAuthError('Tên hiển thị phải từ 2-50 ký tự');
          setAuthLoading(false);
          return;
        }

        // Generate username from display name
        const username = authForm.displayName.toLowerCase().replace(/[^a-z0-9_]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
        
        if (username.length < 3) {
          setAuthError('Tên hiển thị phải có ít nhất 3 ký tự hợp lệ');
          setAuthLoading(false);
          return;
        }
  ...
  return (
    <>
      ...
      >
        ...
        <div 
          className={`relative bg-gray-900 rounded-lg sm:rounded-xl shadow-2xl w-full max-w-sm sm:max-w-md mx-2 sm:mx-4 max-h-[95vh] sm:max-h-[90vh] overflow-y-auto transform ${
            isOpen && isAnimating 
        ...
        {/* Modal Header */}
        <div className={`p-4 sm:p-6 pb-3 sm:pb-4 transition-all duration-300 delay-100 ${
          ...
          <p className="text-gray-300 text-xs sm:text-sm">
            {authMode === 'login' ? (
              ...
              'Tạo tài khoản để lưu danh sách yêu thích và lịch sử xem phim'
            ) : (
        ...
        {/* Tab Navigation */}
        <div className={`px-6 pb-4 transition-all duration-300 delay-150 ${
          isAnimating ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
        }`}>
        ...
        <div className={`px-6 pb-6 transition-all duration-300 delay-150 ${
          ...
          {authMode === 'forgot' ? (
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              >
                {authLoading ? 'Đang gửi...' : 'Gửi yêu cầu'}
              </button>
            </form>
          ) : (
            ...
            <div className="animate-fadeInUp">
              <input
                type="password"
                name="password"
                placeholder={authMode === 'login' ? "Mật khẩu" : "Mật khẩu (ít nhất 6 ký tự, có chữ hoa, thường và số)"}
            ...
            >
              {authLoading ? (
                <div className="flex items-center justify-center space-x-2">
                  <div className="animate-spin rounded-full h-4 w-4 border-2 border-gray-700 border-t-red-500"></div>
                  <span>{authMode === 'login' ? 'Đang đăng nhập...' : 'Đang đăng ký...'}</span>
                </div>
              ) : (
            ...
            {authMode === 'login' && (
              <div className="text-center space-y-2 animate-fadeInUp">
                ...
                <div className="text-xs text-gray-300 space-y-1">
                  ...
                  <div><strong>Member:</strong> member@tuphim.com / Member123</div>
                </div>
              </div>
            )}
