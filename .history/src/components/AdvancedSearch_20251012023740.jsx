      ...
      try {
        console.log('Using instant search API with safe error handling');
         ...
         if (newFilters.keyword) {
           ...
           const pagesToFetch = newFilters.type ? 20 : 10; // More pages for better filtering
           ...
           for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
             ...
             batchResults.forEach(({ page, data, success }) => {
               if (success && data && data.data && data.data.data && data.data.data.items && Array.isArray(data.data.data.items)) {
                 allMovies = allMovies.concat(data.data.data.items);
                 totalPagesResult = data.data.data.params?.pagination?.totalPages || 1;
                 totalItemsResult = data.data.data.params?.pagination?.totalItems || allMovies.length;
                 console.log(`✅ Page ${page}: ${data.data.data.items.length} movies (Total: ${allMovies.length})`);
               }
             });
          ...
          data = result.data;
           ...
           // Use parallel fetching for speed like KKPhim.com
           const pagesToFetch = newFilters.type ? 25 : 15; // More pages for better filtering
           const batchSize = 8; // Fetch 8 pages in parallel for new movies
