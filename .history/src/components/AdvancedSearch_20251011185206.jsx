...
const AdvancedSearch = () => {
  ...
  const performSearch = async (page = 1, newFilters = filters, mode = searchMode) => {
    try {
      ...
      // Progressive loading strategy for 24,673 movies
      const hasFilters = newFilters.keyword || newFilters.category || newFilters.country || newFilters.year || newFilters.type;
      
      // Load more aggressively for better user experience
      const pagesToFetch = hasFilters ? 50 : 100; // Fetch more pages for comprehensive coverage
      const itemsPerPage = 50; // Increased from 24 to 50
      
      console.log(`📄 Fetching ${pagesToFetch} page(s) - Has filters: ${hasFilters} (Database: 24,673 movies, Target: ${pagesToFetch * itemsPerPage} movies)`);
        ...
        if (newFilters.keyword) {
          ...
          for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
            ...
            if (result.data && result.data.data && result.data.data.items && Array.isArray(result.data.data.items)) {
              ...
              console.log(`❌ Search page ${currentPage}: No movies found in response`);
            }
          }
          ...
          // Fetch multiple pages for category search
          let allMovies = [];
          let totalPages = 1;
          let totalItems = 0;
          ...
          for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
            ...
            console.log(`📡 Fetching category page ${currentPage}/${pagesToFetch}`);
            ...
            if (result.data && result.data.data && result.data.data.items && Array.isArray(result.data.data.items)) {
              ...
              totalPages = result.data.data.params?.pagination?.totalPages || 1;
              totalItems = result.data.data.params?.pagination?.totalItems || allMovies.length;
              console.log(`📄 Category page ${currentPage}: ${result.data.data.items.length} movies (Total: ${allMovies.length})`);
            } else {
              console.log(`❌ Category page ${currentPage}: No movies found in response`);
            }
          }
          
          // If no movies found in category, fallback to new movies
          if (allMovies.length === 0) {
            console.log(`🔄 No movies found for category "${newFilters.category}", falling back to new movies`);
            
            for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
              console.log(`📡 Fallback: Fetching new movies page ${currentPage}/${pagesToFetch}`);
              const response = await fetch(`http://localhost:3001/api/movies/new?page=${currentPage}&limit=${itemsPerPage}&version=v3`);
              const result = await response.json();
              
              if (result.data && result.data.items && Array.isArray(result.data.items)) {
                allMovies = allMovies.concat(result.data.items);
                totalPages = result.data.pagination?.totalPages || 1;
                totalItems = result.data.pagination?.totalItems || allMovies.length;
                console.log(`📄 Fallback page ${currentPage}: ${result.data.items.length} movies (Total: ${allMovies.length})`);
              }
            }
          }
          ...
          // Fetch multiple pages for country search
          let allMovies = [];
          let totalPages = 1;
          let totalItems = 0;
          ...
          for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
            ...
            console.log(`📡 Fetching country page ${currentPage}/${pagesToFetch}`);
            ...
            if (result.data && result.data.data && result.data.data.items && Array.isArray(result.data.data.items)) {
              ...
              totalPages = result.data.data.params?.pagination?.totalPages || 1;
              totalItems = result.data.data.params?.pagination?.totalItems || allMovies.length;
              console.log(`📄 Country page ${currentPage}: ${result.data.data.items.length} movies (Total: ${allMovies.length})`);
            } else {
              console.log(`❌ Country page ${currentPage}: No movies found in response`);
            }
          }
          
          // If no movies found in country, fallback to new movies
          if (allMovies.length === 0) {
            console.log(`🔄 No movies found for country "${newFilters.country}", falling back to new movies`);
          ...
          // Fetch multiple pages for year search
          let allMovies = [];
          let totalPages = 1;
          let totalItems = 0;
          ...
          for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
            ...
            console.log(`📡 Fetching year page ${currentPage}/${pagesToFetch}`);
            ...
            if (result.data && result.data.data && result.data.data.items && Array.isArray(result.data.data.items)) {
              ...
              totalPages = result.data.data.params?.pagination?.totalPages || 1;
              totalItems = result.data.data.params?.pagination?.totalItems || allMovies.length;
              console.log(`📄 Year page ${currentPage}: ${result.data.data.items.length} movies (Total: ${allMovies.length})`);
            } else {
              console.log(`❌ Year page ${currentPage}: No movies found in response`);
            }
          }
          
          // If no movies found in year, fallback to new movies
          if (allMovies.length === 0) {
            console.log(`🔄 No movies found for year "${newFilters.year}", falling back to new movies`);
          ...
          // Fetch multiple pages for type search
          let allMovies = [];
          let totalPages = 1;
          let totalItems = 0;
          ...
          for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
            const searchParams = new URLSearchParams({
              keyword: 'phim', // Use broader keyword to get more movies
              page: currentPage,
              limit: 50,
              sort_field: newFilters.sort || 'modified_time',
              sort_type: newFilters.sortType || 'desc',
            ...
            console.log(`📡 Fetching type page ${currentPage}/${pagesToFetch}`);
            const response = await fetch(`http://localhost:3001/api/movies/search?${searchParams.toString()}`);
            const result = await response.json();
            ...
            if (result.data && result.data.data && result.data.data.items && Array.isArray(result.data.data.items)) {
              ...
              totalPages = result.data.data.params?.pagination?.totalPages || 1;
              totalItems = result.data.data.params?.pagination?.totalItems || allMovies.length;
              console.log(`📄 Type page ${currentPage}: ${result.data.data.items.length} movies (Total: ${allMovies.length})`);
            } else {
              console.log(`❌ Type page ${currentPage}: No movies found in response`);
            }
          }
          
          // If no movies found in type search, fallback to new movies
          if (allMovies.length === 0) {
            console.log(`🔄 No movies found for type "${newFilters.type}", falling back to new movies`);
          ...
          // Progressive loading for all 24,673 movies
          let allMovies = [];
          let totalPages = 1;
          let totalItems = 0;
          
          console.log(`🚀 Starting progressive load of all 24,673 movies...`);
          
          for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
            console.log(`📡 Fetching new movies page ${currentPage}/${pagesToFetch}`);
            
            // Update progress
            const progress = Math.round((currentPage / pagesToFetch) * 100);
            setLoadingProgress({ percentage: progress, current: currentPage, total: pagesToFetch });
            ...
            if (result.data && result.data.items) {
              ...
              totalPages = result.data.pagination?.totalPages || 1;
              totalItems = result.data.pagination?.totalItems || allMovies.length;
              console.log(`📄 New movies page ${currentPage}: ${result.data.items.length} movies (Total: ${allMovies.length})`);
              
              // Update movies immediately for progressive display
              setMovies(allMovies);
            }
  ...
  const initializeDatabase = async () => {
    try {
      ...
      // If no cache, start preloading
      setDatabaseStatus(prev => ({ ...prev, loading: true }));
      console.log('🚀 Starting database preload...');
      
      await movieDatabase.preloadDatabase();
      
      setDatabaseStatus({
        isLoaded: true,
        totalMovies: movieDatabase.allMovies.length,
        loading: false,
        lastUpdate: movieDatabase.lastUpdate
      });
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="container mx-auto px-6 py-8">
        {/* Progress Indicator */}
        {loading && loadingProgress.percentage > 0 && (
          <div className="mb-8 bg-gray-800 rounded-lg p-4">
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm text-gray-300">
                🚀 Loading movies... {loadingProgress.current}/{loadingProgress.total} pages
              </span>
              <span className="text-sm text-gray-300">
            ...
            <div className="text-xs text-gray-400 mt-1">
              Đang tải {movies.length.toLocaleString()} phim từ database 24,673 phim...
            </div>
          </div>
        )}
        ...
        {loading ? (
          <div className="space-y-6">
            {/* Show current movies while loading more */}
            {movies.length > 0 ? (
