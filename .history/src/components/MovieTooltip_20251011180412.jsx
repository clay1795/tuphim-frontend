...
const MovieTooltip = ({ movie, children }) => {
  ...
  const fetchMovieDetails = async (movieId, movieSlug) => {
    ...
    try {
      // Use backend API instead of direct phimapi.com call
      let apiUrl;
      if (movieSlug) {
        apiUrl = `http://localhost:3001/api/movies/detail/${movieSlug}`;
      } else if (movieId) {
        apiUrl = `http://localhost:3001/api/movies/detail/${movieId}`;
      } else {
      ...
      if (response.ok) {
        ...
        if (data && data.success && data.data) {
          ...
          console.log('🎬 Movie data received from backend:', {
            name: movieData.name,
            year: movieData.year,
          ...
          setEnhancedMovie(prev => ({
            ...
            // Use original image URLs from the initial movie data (they're correct)
            poster_url: prev.poster_url || movieData.poster_url,
            thumb_url: prev.thumb_url || movieData.thumb_url,
            banner_url: prev.banner_url || movieData.banner_url
          }));
        ...
        >
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-2">
              ...
              {/* Episode status or Duration - Smart display */}
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-xs text-gray-400">
                    {(() => {
                      ...
                      console.log('🎬 Single Movie Detection:', {
                        type: enhancedMovie.type,
                        episode_total: enhancedMovie.episode_total,
                        category: enhancedMovie.category?.map(c => c.name),
                        isSingleMovie
                      });
                      
                      return isSingleMovie ? 'Thời lượng:' : 'Tập hiện tại:';
                    })()}
                  </span>
              ...
              {/* Added time info for user tabs - moved to bottom */}
