...
const SeparatedMovieCard = ({ movie, index = 0 }) => {
  ...
  const imageUrl = movieApi.getWebpImage(movie.poster_url || movie.thumb_url);
  console.log(`🖼️ Movie "${movie.name}" - poster_url: ${movie.poster_url}, thumb_url: ${movie.thumb_url}, final URL: ${imageUrl}`);
  
  // Validate URL format
  if (imageUrl && imageUrl.includes('localhost:5173')) {
    console.error(`❌ Invalid URL detected for "${movie.name}": ${imageUrl}`);
  }
        ...
        <div className="relative overflow-hidden rounded-xl shadow-2xl hover:shadow-3xl transition-all duration-500 group-hover:scale-105 group-hover:-translate-y-2">
          <div className="relative aspect-[3/4] overflow-hidden">
            ...
            <img
              ...
              loading="lazy"
              onLoad={() => {
                console.log('✅ Image loaded successfully for:', movie.name, 'URL:', imageUrl);
              }}
              onError={(e) => {
                console.log('❌ Image load error for:', movie.name, 'URL:', imageUrl);
                
                // Try fallback to thumb_url if poster_url failed
                if (movie.poster_url && movie.thumb_url && e.target.src.includes(movie.poster_url)) {
                  console.log('🔄 Trying fallback to thumb_url:', movie.thumb_url);
                  e.target.src = movieApi.getWebpImage(movie.thumb_url);
                  return;
                }
