...
const MovieTooltip = ({ movie, children }) => {
  ...
  const formatEpisodeStatus = (episodeCurrent, episodeTotal, status) => {
    ...
    // Check if episode_current already contains "Tập" text
    const hasTậpInCurrent = episodeCurrent.toString().includes('Tập') || episodeCurrent.toString().includes('tập');
    const hasTậpInTotal = episodeTotal && episodeTotal.toString().includes('Tập');
    
    // If it's a completed series
    if (status === 'completed' || status === 'hoàn thành' || (episodeTotal && episodeCurrent === episodeTotal)) {
      if (hasTậpInCurrent) {
        return `Hoàn Tất ${episodeCurrent}`;
      }
      return `Hoàn Tất (${episodeCurrent}/${episodeTotal || episodeCurrent})`;
    }
    
    // If it's currently airing
    if (hasTậpInCurrent) {
      return episodeCurrent; // Just show as is if already has "Tập"
    }
    
    return `Tập ${episodeCurrent}`;
  };
        ...
        >
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-2">
              ...
              {/* Episode status - Smart display */}
              {enhancedMovie.episode_current && (
                <div className="flex items-center space-x-2">
                  <span className="text-xs text-gray-400">Tập hiện tại:</span>
                  <span className={`text-xs px-2 py-1 rounded-full border ${
                    // Check if it's completed
                    (enhancedMovie.status === 'completed' || enhancedMovie.status === 'hoàn thành' || 
                     (enhancedMovie.episode_total && enhancedMovie.episode_current === enhancedMovie.episode_total))
                      ? 'bg-green-600/20 text-green-400 border-green-600/30'
                      : 'bg-orange-600/20 text-orange-400 border-orange-600/30'
                  }`}>
                    {formatEpisodeStatus(enhancedMovie.episode_current, enhancedMovie.episode_total, enhancedMovie.status)}
                  </span>
                </div>
              )}
