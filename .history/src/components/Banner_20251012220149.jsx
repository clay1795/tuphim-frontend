...
const Banner = ({ onFeaturedMovieChange }) => {
  ...
  const getDynamicFontStyle = (movie) => {
    // Lấy URL thumbnail để phân tích
    const thumbUrl = movie.thumb_url || movie.poster_url;
    
    // Tạo hash từ URL để có consistency
    const hash = thumbUrl ? thumbUrl.split('').reduce((a, b) => {
      a = ((a << 5) - a) + b.charCodeAt(0);
      return a & a;
    }, 0) : 0;
    
    // Phân tích dựa trên hash và đặc điểm thumbnail
    const styleIndex = Math.abs(hash) % 8;
    
    // Kiểm tra URL cụ thể cho phim "Hãy lấy em đi"
    if (thumbUrl && thumbUrl.includes('0104efe6e144dc802ba4d74ebb2df3e6')) {
      // Style đặc biệt cho phim "Hãy lấy em đi" với màu sắc từ thumbnail
      return {
        fontFamily: 'Impact, Arial Black, sans-serif',
        fontWeight: '900',
        color: '#FF6B9D', // Màu hồng từ thumbnail
        textShadow: '3px 3px 0px #C2185B, 6px 6px 15px rgba(255,107,157,0.8), 0 0 25px rgba(255,107,157,0.6)',
        letterSpacing: '2px',
        transform: 'scale(1.05)',
        filter: 'drop-shadow(0 0 15px rgba(255,107,157,0.7)) brightness(1.1)',
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/40 to-black/10"></div>
               ...
               <div className="relative h-full">
                 ...
                 <div className="absolute left-3 sm:left-6 top-1/2 -translate-y-1/2 z-20">
                   <div key={featuredMovie._id} className="p-3 sm:p-6 max-w-xs sm:max-w-md animate-fadeInUp">
