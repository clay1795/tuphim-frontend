...
import { mongoMovieApi } from '../services/mongoMovieApi';
  ...
  useEffect(() => {
    const loadTrendingMovies = async () => {
      try {
        ...
        // Lấy phim mới cập nhật để làm trending
        const data = await movieApi.getNewMoviesV3(1);
  ...
  useEffect(() => {
    const loadFavoriteMovies = async () => {
      try {
        ...
        // Lấy phim phổ biến từ nhiều nguồn để làm favorite
        console.log('Loading favorite movies from multiple sources...');
        
        // Thử lấy từ phim mới cập nhật (trang 2 để có phim khác)
        let data = await movieApi.getNewMoviesV3(2);
        console.log('Favorite movies API response (page 2):', data);
        
        // Nếu không có dữ liệu, thử trang 3
        if (!data || (Array.isArray(data) ? data.length === 0 : Object.keys(data).length === 0)) {
          console.log('Trying page 3 for favorite movies...');
          data = await movieApi.getNewMoviesV3(3);
        }
        
        // Xử lý dữ liệu API từ nhiều nguồn
        let movieData = [];
        const possibleDataKeys = ['items', 'data', 'movies', 'results', 'content', 'list', 'docs', 'records'];
        ...
        for (const key of possibleDataKeys) {
          if (data[key] && Array.isArray(data[key])) {
            movieData = data[key];
            console.log(`Found favorite movies in key "${key}":`, movieData.length, 'items');
            break;
          }
        }
        ...
        // Nếu vẫn không có dữ liệu, thử lấy từ thể loại khác
        if (movieData.length === 0) {
          console.log('Trying alternative API sources for favorite movies...');
          try {
            // Thử lấy phim từ thể loại phổ biến
            const alternativeData = await movieApi.getMoviesByCategory('tinh-cam', { page: 1 });
            console.log('Alternative favorite movies (tinh-cam):', alternativeData);
            
            for (const key of possibleDataKeys) {
              if (alternativeData[key] && Array.isArray(alternativeData[key])) {
                movieData = alternativeData[key];
                console.log(`Found alternative movies in key "${key}":`, movieData.length, 'items');
                break;
              }
            }
            
            if (movieData.length === 0 && Array.isArray(alternativeData)) {
              movieData = alternativeData;
            }
            
            // Nếu vẫn không có, thử thể loại khác
            if (movieData.length === 0) {
              console.log('Trying action movies as alternative...');
              const actionData = await movieApi.getMoviesByCategory('hanh-dong', { page: 1 });
              console.log('Action movies for favorites:', actionData);
              
              for (const key of possibleDataKeys) {
                if (actionData[key] && Array.isArray(actionData[key])) {
                  movieData = actionData[key];
                  console.log(`Found action movies in key "${key}":`, movieData.length, 'items');
                  break;
                }
              }
              
              if (movieData.length === 0 && Array.isArray(actionData)) {
                movieData = actionData;
              }
            }
          } catch (altErr) {
            console.log('Alternative API failed, using fallback:', altErr);
          }
        }
  ...
  // Load top comments from API
  useEffect(() => {
    const loadTopComments = async () => {
      try {
        setLoadingComments(true);
        console.log('Loading top comments from API...');
        
        // Lấy phim mới để tạo bình luận
        const data = await movieApi.getNewMoviesV3(1);
        console.log('Top comments API response:', data);
  ...
  // Load latest comments from API
  useEffect(() => {
    const loadLatestComments = async () => {
      try {
        setLoadingLatest(true);
        console.log('Loading latest comments from API...');
        
        // Lấy phim từ trang khác để có dữ liệu mới
        const data = await movieApi.getNewMoviesV3(2);
        console.log('Latest comments API response:', data);
  ...
  const refreshLatestComments = async () => {
    try {
      ...
      const data = await movieApi.getNewMoviesV3(randomPage);
    ...
    <>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <section style={{
          ...
          border: `1px solid ${sectionBackgroundColor.replace('0.15', '0.3')}`,
          borderRadius: '14px',
          boxShadow: `0 0 0.6rem ${sectionBackgroundColor.replace('0.15', '0.2')}`,
          padding: '18px',
          marginBottom: '22px',
          ...
          <div style={{
            ...
            {loadingComments ? (
              ...
              <article key={comment.id} style={{
                ...
                {/* Nội dung chính với background trong suốt */}
                <div style={{
                  position: 'relative',
                  zIndex: 1,
                  display: 'flex',
                  ...
                  <div style={{
                    ...
                    <img 
                      ...
                      onError={(e) => {
                        ...
                        const movieName = comment.title || 'Unknown';
                        e.target.src = `https://via.placeholder.com/50x70/2a2a2a/ffffff?text=${encodeURIComponent(movieName.substring(0, 3))}`;
                      }}
                      onLoad={(e) => {
                  ...
                  <div style={{
                    ...
                    <span><i className="fa-solid fa-thumbs-up arrow-up" style={{ color: '#666' }}></i> {comment.upvotes}</span>
                    <span><i className="fa-solid fa-thumbs-down arrow-down" style={{ color: '#666' }}></i> {comment.downvotes}</span>
                    <span><i className="fa-regular fa-comment" style={{ color: '#666' }}></i> {comment.replies}</span>
                  </div>
                </div>
              </article>
            ))
