...
const MovieTooltip = ({ movie, children }) => {
  ...
  const handleAddToFavorites = async (e) => {
    ...
    try {
      const result = await addToFavorites({
        ...
        movieSlug: movie.slug || movie.movieSlug,
        movieName: movie.name || movie.movieName,
        poster_url: movie.poster_url,
        thumb_url: movie.thumb_url,
        banner_url: movie.banner_url,
        originalName: movie.original_name || movie.originalName || movie.name || movie.movieName
      });
  ...
  const fetchMovieDetails = async (movieId, movieSlug) => {
    ...
    try {
      // Try to fetch from movieApi first
      const response = await movieApi.getMovieDetails(movieSlug || movieId);
      if (response && response.data) {
        setEnhancedMovie(response.data);
        return;
      }
    } catch (error) {
      console.log('Failed to fetch from movieApi, trying direct API...');
    }
    
    try {
      // Fallback: try direct API call
      const apiUrl = `https://phimapi.com/v1/api/item/get?slug=${movieSlug || movieId}`;
      const response = await fetch(apiUrl);
      if (response.ok) {
        const data = await response.json();
        if (data && data.data && data.data.movie) {
          setEnhancedMovie(data.data.movie);
        }
      }
    } catch (error) {
  ...
  const handleMouseEnter = () => {
    ...
    const needsEnhancement = !enhancedMovie.rating && 
                            !enhancedMovie.year && 
                            !enhancedMovie.duration &&
                            !enhancedMovie.category &&
                            !enhancedMovie.description;
    
    if (needsEnhancement && (enhancedMovie.slug || enhancedMovie.movieSlug || enhancedMovie._id || enhancedMovie.movieId)) {
  ...
  e.target.parentElement.style.background = 'linear-gradient(135deg, #374151 0%, #4b5563 100%)';
          ...
          <div className="p-4 space-y-3">
            ...
            <div>
              ...
              {(movie.name_en || movie.originalName) && (
            ...
            <div className="flex space-x-2">
              <Link
                to={`/phim/${movie.slug || movie.movieSlug || movie._id || movie.movieId}`}
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-4 text-sm">
                {(movie.rating || movie.content_rating) && (
                  <div className="flex items-center space-x-1">
                    {getRatingBadge(movie.rating || movie.content_rating)}
                  </div>
                )}
                
                {(movie.year || movie.release_year) && (
                  <span className="text-white font-medium">
                    {formatYear(movie.year || movie.release_year)}
                  </span>
                )}
                
                {(movie.duration || movie.runtime) && (
                  <span className="text-white font-medium">
                    {getDuration(movie.duration || movie.runtime)}
                  </span>
                )}
                
                {/* Fallback info if no metadata available */}
                {!movie.rating && !movie.year && !movie.duration && !movie.content_rating && !movie.release_year && !movie.runtime && (
              ...
              {movie.addedAt && (
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs text-gray-300">
                    {new Date(movie.addedAt).toLocaleDateString('vi-VN')}
                  </span>
                </div>
              )}
              ...
              {movie.watchedAt && (
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs text-gray-300">
                    {new Date(movie.watchedAt).toLocaleDateString('vi-VN')}
                  </span>
                </div>
              )}
