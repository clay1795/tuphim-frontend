...
const MovieTooltip = ({ movie, children }) => {
  ...
  const updatePosition = () => {
    if (triggerRef.current && tooltipRef.current) {
      ...
      // For relative positioning, center tooltip over the trigger element
      let x = triggerRect.width / 2 - tooltipRect.width / 2;
      let y = triggerRect.height / 2 - tooltipRect.height / 2;

      // Adjust if tooltip goes off screen horizontally (relative to trigger)
      if (x < -triggerRect.left + 20) {
        x = -triggerRect.left + 20;
      }
      if (x + tooltipRect.width > viewportWidth - triggerRect.left - 20) {
        x = viewportWidth - triggerRect.left - tooltipRect.width - 20;
      }

      // Adjust if tooltip goes off screen vertically (relative to trigger)
      if (y < -triggerRect.top + 20) {
        y = -triggerRect.top + 20;
      }
      if (y + tooltipRect.height > viewportHeight - triggerRect.top - 20) {
        y = viewportHeight - triggerRect.top - tooltipRect.height - 20;
      }

      console.log('🎯 Final Position (Relative):', { x, y });
  ...
  return (
    <div className="relative inline-block">
      ...
      {isVisible && (
        <div
          ref={tooltipRef}
          className="absolute w-96 rounded-xl shadow-2xl overflow-hidden movie-tooltip hover-lift"
          style={{
            left: `${position.x}px`,
            top: `${position.y}px`,
            pointerEvents: 'auto',
            zIndex: 2147483647,
            animation: 'tooltipSlideIn 0.25s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards',
        ...
        >
          ...
          {/* Movie Thumbnail - Optimized animation */}
          ...
          <div className="p-4 space-y-3">
            {/* Movie Title */}
            <div>
              <h3 className="text-white font-bold text-lg mb-1 line-clamp-2 transition-colors duration-200 hover:text-yellow-400">
                {enhancedMovie.name || enhancedMovie.movieName}
              </h3>
              {(enhancedMovie.name_en || enhancedMovie.originalName || enhancedMovie.origin_name) && (
                <p className="text-yellow-400 text-sm font-medium transition-colors duration-200 hover:text-yellow-300">
                  {enhancedMovie.name_en || enhancedMovie.originalName || enhancedMovie.origin_name}
                </p>
              )}
              {/* Loading indicator */}
            ...
            {/* Action Buttons - Optimized animations */}
            <div className="flex items-stretch space-x-2">
              ...
              <Link
                ...
                className="flex-grow bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-2 px-6 rounded-lg transition-colors duration-200 flex items-center justify-center space-x-2 hover:scale-105"
              ...
              <button 
                ...
                className="flex-shrink-0 bg-gray-700 hover:bg-gray-600 text-white font-medium py-2 px-3 rounded-lg border border-gray-600 transition-colors duration-200 flex items-center justify-center space-x-1 hover:scale-105"
...
// CSS Animation Styles - Optimized for smooth performance
const tooltipStyles = `
  ...
  /* Force tooltip to be always on top - highest priority */
  .movie-tooltip {
    position: fixed !important;
    z-index: 2147483647 !important;
...
if (typeof document !== 'undefined') {
  ...
  const existingStyle = document.getElementById('movie-tooltip-styles');
  if (!existingStyle) {
    const styleElement = document.createElement('style');
    styleElement.id = 'movie-tooltip-styles';
