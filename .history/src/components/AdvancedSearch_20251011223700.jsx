...
const AdvancedSearch = () => {
  ...
  const initializeDatabase = async () => {
    try {
      ...
      // Try to load real movies from API
      try {
        const response = await fetch('http://localhost:3001/api/movies/new?page=1&limit=50&version=v3');
        const result = await response.json();
        
        if (result.success && result.data && result.data.items && result.data.items.length > 0) {
          console.log('✅ Loaded real movies from API:', result.data.items.length);
          
          // Set movies directly without database
          setMovies(result.data.items);
          setTotalResults(result.data.items.length);
          setTotalPages(result.data.pagination?.totalPages || 1);
          
          setDatabaseStatus({
            isLoaded: true,
            totalMovies: result.data.items.length,
            loading: false,
            lastUpdate: new Date()
          });
          
          console.log('🎯 Real movies displayed:', result.data.items.length);
        } else {
