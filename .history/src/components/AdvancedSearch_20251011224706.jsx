...
const AdvancedSearch = () => {
  ...
  const loadFilterData = async () => {
    try {
      ...
      if (categoriesResult.success && Array.isArray(categoriesResult.data)) {
        ...
        console.warn('Categories API returned invalid data:', categoriesResult);
        setCategories([]);
      }
      ...
      if (countriesResult.success && Array.isArray(countriesResult.data)) {
        ...
        console.warn('Countries API returned invalid data:', countriesResult);
        setCountries([]);
      }
      
      console.log('Loaded filters:', {
        categories: Array.isArray(categoriesResult.data) ? categoriesResult.data.length : 0,
        countries: Array.isArray(countriesResult.data) ? countriesResult.data.length : 0
      });
    } catch (error) {
  ...
  const performSearch = async (page = 1, newFilters = filters, mode = searchMode) => {
    try {
      ...
      const pagesToFetch = hasFilters ? 5 : 10; // Fetch more pages when no filters to show all movies
      ...
      try {
        ...
        if (newFilters.keyword) {
          ...
          for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
            ...
            console.log(`🔍 Search page ${currentPage} response:`, {
              success: result.success,
              hasData: !!result.data,
              hasDataData: !!(result.data && result.data.data),
              hasItems: !!(result.data && result.data.data && result.data.data.items),
          ...
          // If no movies found in keyword search, fallback to new movies
          if (allMovies.length === 0) {
            console.log(`🔄 No movies found for keyword "${newFilters.keyword}", falling back to new movies`);
          ...
          data = {
           ...
           if (allMovies.length === 0) {
             console.log(`🔄 No movies found for type "${newFilters.type}", falling back to new movies and filtering by type`);
