      ...
      try {
        console.log('Using instant search API with safe error handling');
         ...
         if (newFilters.keyword) {
           ...
           const pagesToFetch = newFilters.type ? 10 : 3; // More pages when filtering by type
           let allMovies = [];
           ...
           for (let currentPage = 1; currentPage <= pagesToFetch; currentPage++) {
             ...
             try {
               ...
               if (result.data && result.data.data && result.data.data.items && Array.isArray(result.data.data.items)) {
                 ...
                 totalPagesResult = result.data.data.params?.pagination?.totalPages || 1;
                 totalItemsResult = result.data.data.params?.pagination?.totalItems || allMovies.length;
                 console.log(`📄 Search page ${currentPage}: ${result.data.data.items.length} movies (Total: ${allMovies.length})`);
               }
