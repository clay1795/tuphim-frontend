            ...
            try {
              ...
              if (commentResponse.success && commentResponse.data.length > 0) {
                ...
                commentResponse.data.forEach(comment => {
                  ...
                  // Luôn hiển thị fullName - tất cả user đều có fullName
        ...
        try {
          ...
          if (commentResponse.success && commentResponse.data.length > 0) {
            commentResponse.data.forEach(comment => {
              ...
              // Hiển thị fullName nếu có (và không rỗng), không thì dùng username
              const displayName = (comment.fullName && comment.fullName.trim()) || comment.username || 'User';
              const avatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=007bff&color=fff&size=32&bold=true`;
