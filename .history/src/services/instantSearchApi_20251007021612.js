...
const applySorting = (results, sortField, sortType, searchType = '') => {
  ...
  const sortedResults = [...results].sort((a, b) => {
    ...
    switch (sortField) {
      case 'modified_time':
        aValue = new Date(a.modified_time || a.updatedAt || a.createdAt || 0);
        bValue = new Date(b.modified_time || b.updatedAt || b.createdAt || 0);
        break;
      case 'created_time':
        aValue = new Date(a.created_time || a.createdAt || a.modified_time || 0);
        bValue = new Date(b.created_time || b.createdAt || b.modified_time || 0);
        break;
      case 'year':
      ...
      case 'name':
        ...
        break;
      case 'view':
        aValue = parseInt(a.view || a.views || a.view_count || 0);
        bValue = parseInt(b.view || b.views || b.view_count || 0);
        break;
      default:
        return 0;
    }
  ...
  if (sortedResults.length > 0) {
    const sample = sortedResults.slice(0, 3);
    console.log(`Sample after sorting by ${sortField} (${sortType}):`, sample.map(movie => ({
      name: movie.name,
      year: movie.year,
...
const applyFiltersToResults = (results, keyword, options, searchType = '') => {
  ...
  // Apply sorting based on API-style sorting
  if (options.sort && options.sort.trim()) {
    console.log(`Applying sort: ${options.sort} (${options.sortType}) to ${filteredResults.length} movies`);
    
    filteredResults.sort((a, b) => {
      const sortField = options.sort;
      const sortOrder = options.sortType || 'desc';
    ...
    // Debug: Show sample before and after sorting
    if (filteredResults.length > 0) {
      const sample = filteredResults.slice(0, 3);
      console.log(`Sample after sorting by ${options.sort} (${options.sortType}):`, sample.map(movie => ({
        name: movie.name,
        year: movie.year,
        modified_time: movie.modified_time,
        created_time: movie.created_time,
    ...
    console.log(`${searchType} - After sorting by "${options.sort}" (${options.sortType}): ${filteredResults.length} movies`);
  }
    ...
    try {
      ...
      for (let page = 1; page <= 10; page++) {
        preloadPromises.push(
          simpleMovieApi.getNewMovies(page, 'v3', { sort: 'modified_time', sortType: 'desc' })
            .then(data => {
              let movieData = [];
  ...
  loadFullDatabase: async (onProgress = null) => {
    try {
      ...
      while (currentPage <= estimatedPages && consecutiveErrors < maxConsecutiveErrors) {
        ...
        batchPromises.push(
          simpleMovieApi.getNewMovies(currentPage, 'v3', { sort: 'modified_time', sortType: 'desc' })
            .then(data => {
                let movieData = [];
                if (data.items && Array.isArray(data.items)) {
                  movieData = data.items;
