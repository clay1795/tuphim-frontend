...
// Cache system for faster search
const searchCache = new Map();
const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes cache

// Cache helper functions
const getCacheKey = (keyword, options) => {
  return `${keyword}_${JSON.stringify(options)}`;
};
...
export const simpleMovieApi = {
  // Tìm kiếm instant cho từ khóa phổ biến
  instantSearch: async (keyword) => {
    ...
    const normalizedKeyword = instantKeywords[keyword.toLowerCase()] || keyword;
    console.log(`Instant search: "${keyword}" -> "${normalizedKeyword}"`);
    
    // Tìm kiếm nhanh trong 5 trang đầu
    let results = [];
    const instantPages = 5;
    
    for (let page = 1; page <= instantPages; page++) {
      try {
        const pageData = await simpleMovieApi.getNewMovies(page);
        if (pageData.items) {
          const filtered = pageData.items.filter(movie => {
            const movieName = (movie.name || '').toLowerCase();
            const originName = (movie.origin_name || '').toLowerCase();
            const slug = (movie.slug || '').toLowerCase();
            
            return movieName.includes(normalizedKeyword.toLowerCase()) ||
                   originName.includes(normalizedKeyword.toLowerCase()) ||
                   slug.includes(normalizedKeyword.toLowerCase());
          });
          results = [...results, ...filtered];
        }
      } catch (error) {
        console.error(`Instant search page ${page} error:`, error);
      }
    }
    
    return {
      items: results.slice(0, 20), // Giới hạn 20 kết quả
  ...
  // Tìm kiếm phim tối ưu tốc độ cho hosting
  searchMovies: async (keyword, options = {}) => {
    try {
      ...
      const cacheKey = getCacheKey(keyword, { category, country, year });
      ...
      console.log(`Fast search with keyword: "${keyword}", filters:`, { category, country, year });
      
      // Tìm kiếm song song nhiều trang để tăng tốc
      let allMovies = [];
      const searchPages = 15; // Giảm xuống 15 trang nhưng tìm kiếm song song
      const batchSize = 5; // Tìm kiếm 5 trang cùng lúc
      
      // Tạo các batch tìm kiếm
      const batches = [];
      for (let i = 1; i <= searchPages; i += batchSize) {
        const batch = [];
        for (let j = i; j < Math.min(i + batchSize, searchPages + 1); j++) {
          batch.push(j);
        }
        batches.push(batch);
      }
      
      // Tìm kiếm từng batch song song
      for (const batch of batches) {
        try {
          console.log(`Searching batch: pages ${batch.join(', ')}...`);
          
          // Tìm kiếm song song tất cả trang trong batch
          const batchPromises = batch.map(async (currentPage) => {
            try {
              const pageData = await simpleMovieApi.getNewMovies(currentPage);
              
              if (pageData.items && Array.isArray(pageData.items)) {
                // Lọc phim với thuật toán tối ưu
                return pageData.items.filter(movie => {
                  return simpleMovieApi.quickSearchFilter(movie, keyword, { category, country, year });
                });
              }
              return [];
            } catch (pageError) {
          ...
          // Chờ tất cả trang trong batch hoàn thành
          ...
          console.log(`Batch completed: Found ${allMovies.length} total movies so far`);
          
          // Dừng sớm nếu đã đủ kết quả
          if (allMovies.length >= limit * 3) {
      ...
      // Lưu kết quả vào cache (chỉ lưu trang đầu tiên)
      if (page === 1) {
        setToCache(cacheKey, uniqueMovies);
      }
  ...
  // Bộ lọc tìm kiếm nhanh và tối ưu
  quickSearchFilter: (movie, keyword, filters) => {
    const keywordLower = keyword.toLowerCase().trim();
    
    // Tìm kiếm nhanh trong các trường chính
    const movieName = (movie.name || '').toLowerCase();
    const originName = (movie.origin_name || '').toLowerCase();
    const slug = (movie.slug || '').toLowerCase();
    ...
    // 1. Khớp chính xác trong tên phim (ưu tiên cao nhất)
    if (movieName.includes(keywordLower) || originName.includes(keywordLower)) {
      matched = true;
    }
    
    // 2. Khớp trong slug
    else if (slug.includes(keywordLower)) {
      matched = true;
    }
    
    // 3. Khớp từng từ trong tên phim
    else {
    ...
    // 4. Smart search nhanh
    if (!matched) {
      matched = simpleMovieApi.quickSmartSearch(keywordLower, movie);
    }
    ...
    if (filters.category) {
      ...
      if (!categoryNames.includes(filters.category.toLowerCase())) {
        return false;
      }
    }
    ...
    if (filters.country) {
      ...
      if (!countryNames.includes(filters.country.toLowerCase())) {
        return false;
      }
    }
    ...
    if (filters.year) {
      ...
      if (movieYear.toString() !== filters.year.toString()) {
        return false;
      }
    }
  ...
  // Smart search nhanh
  quickSmartSearch: (keyword, movie) => {
    const movieName = (movie.name || '').toLowerCase();
    const originName = (movie.origin_name || '').toLowerCase();
    const movieCountries = movie.country || [];
    const countryNames = movieCountries.map(country => (country.name || country).toLowerCase()).join(' ');
    ...
    // Mapping nhanh cho các từ khóa phổ biến
    const quickMappings = {
      'vietnam': ['việt nam', 'vietnam', 'vn'],
      'korea': ['hàn quốc', 'korea', 'korean'],
      'china': ['trung quốc', 'china', 'chinese'],
      'japan': ['nhật bản', 'japan', 'japanese'],
    ...
    // Kiểm tra mapping nhanh
    for (const [key, variations] of Object.entries(quickMappings)) {
