...
export const simpleMovieApi = {
  ...
  getNewMovies: async (page = 1, version = 'v3', sortOptions = {}) => {
    try {
      ...
      const response = await fetch(`${API_CONFIG.getApiUrl()}/movies/new?page=${page}&version=${version}`, {
        method: 'GET',
        headers: {
  ...
  getMovieDetail: async (slug) => {
    try {
      ...
      const response = await fetch(`${API_CONFIG.getApiUrl()}/movies/detail/${slug}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
