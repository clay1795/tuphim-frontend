...
// Tài liệu: https://kkphim.com/tai-lieu-api

const KKPHIM_API_BASE = 'https://phimapi.com';
const KKPHIM_IMG_BASE = 'https://phimimg.com';
const KKPHIM_PLAYER_BASE = 'https://player.phimapi.com/player/?url=';
const CACHE_DURATION = 5 * 60 * 1000; // 5 phút cache
const MAX_REQUESTS_PER_MINUTE = 100; // KKPhim cho phép nhiều requests hơn
...
class KKPhimApiService {
  ...
  async getNewMovies(page = 1, version = 'v3') {
    const endpoint = `/danh-sach/phim-moi-cap-nhat-${version}`;
  ...
  async getMoviesList(typeList, options = {}) {
    ...
    const endpoint = `/v1/api/danh-sach/${typeList}`;
    const params = {
      page,
      sort_field,
      sort_type,
  ...
  async searchMovies(keyword, options = {}) {
    ...
    const endpoint = `/v1/api/tim-kiem`;
    const params = {
      keyword,
      page,
      sort_field,
  ...
  async getCategoryDetail(slug, options = {}) {
    ...
    const endpoint = `/v1/api/the-loai/${slug}`;
    const params = {
      page,
      sort_field,
      sort_type,
