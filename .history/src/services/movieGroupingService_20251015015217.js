...
class MovieGroupingService {
  ...
  createSeriesKey(name, originName) {
    ...
    let cleanName = baseName
      // Loại bỏ các pattern phần/phần mùa với số
      .replace(/\s+(phần|part|season|mùa|season)\s*\d+.*$/i, '')
      .replace(/\s+\d+.*$/i, '') // Loại bỏ số ở cuối (có thể là phần)
      .replace(/\s+\d{4}.*$/i, '') // Loại bỏ năm
      // Loại bỏ chất lượng
      .replace(/\s+(hd|fhd|4k|cam|ts|tc|full hd|ultra hd).*$/i, '')
      // Loại bỏ ngôn ngữ
      .replace(/\s+(vietsub|thuyết minh|lồng tiếng|sub|dub).*$/i, '')
