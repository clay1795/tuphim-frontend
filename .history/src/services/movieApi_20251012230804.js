...
export const movieApi = {
  ...
  getMoviesByCategory: async (type, page = 1, category = '', country = '', year = '', limit = 24) => {
  ...
  getMovieDetail: async (slug) => {
    try {
      console.log('Fetching movie detail from:', `${BASE_URL}/movies/detail/${slug}`);
      const response = await fetch(`${BASE_URL}/movies/detail/${slug}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
  ...
  getWebpImage: (imageUrl) => {
    ...
    // Sử dụng URL gốc trực tiếp vì phimimg.com/image.php không hoạt động
    return imageUrl;
  },
