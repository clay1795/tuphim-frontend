...
export const createDetailedMovieSections = (movies) => {
  ...
  sectionConfigs.forEach((config) => {
    ...
    // Lấy tối đa 12 phim từ bộ lọc chính
    const primarySelected = primaryMovies.slice(0, 12);
    ...
    // Bước 2: Nếu chưa đủ 12 phim, thử bộ lọc linh hoạt hơn
    ...
    // Bước 3: Nếu vẫn chưa đủ 12 phim, thử bộ lọc rất linh hoạt
    if (filteredMovies.length < 12) {
      const veryFlexibleCriteria = {
        countries: ['Tất cả'],
        movieType: 'Tất cả',
        genres: ['Tất cả'],
        years: ['Tất cả'],
      ...
      const flexibleMovies = filterMoviesByDetailedCriteria(movies, veryFlexibleCriteria)
        .filter(movie => {
          const movieId = movie._id || movie.id || movie.slug || movie.name;
          return !usedMovieIds.has(movieId);
        })
      ...
      console.log(`Flexible movies found: ${flexibleMovies.length}`);
      if (flexibleMovies.length > 0) {
        console.log('Sample flexible movies:', flexibleMovies.slice(0, 3).map(m => `${m.name} (${m.year})`));
      }
      
      flexibleMovies.forEach(movie => {
        const movieId = movie._id || movie.id || movie.slug || movie.name;
        usedMovieIds.add(movieId);
      });
      filteredMovies = [...filteredMovies, ...flexibleMovies];
    }
