...
// Bộ lọc chi tiết cho các section
export const filterMoviesByDetailedCriteria = (movies, criteria) => {
  ...
  // Lọc theo loại phim
  if (criteria.movieType) {
    if (criteria.movieType === 'single') {
    ...
    } else if (criteria.movieType === 'series') {
      filteredMovies = filteredMovies.filter(movie => {
  ...
  if (criteria.years && criteria.years.length > 0) {
    filteredMovies = filteredMovies.filter(movie => {
      ...
      return criteria.years.includes(movieYear);
    });
  }
  
  // Lọc theo quốc gia (nếu không phải "Tất cả")
  if (criteria.countries && criteria.countries.length > 0 && !criteria.countries.includes('all')) {
    filteredMovies = filteredMovies.filter(movie => {
      ...
      return criteria.countries.some(country => 
        movieCountries.some(movieCountry => 
          movieCountry.includes(country.toLowerCase()) || 
          country.toLowerCase().includes(movieCountry)
        )
      );
    });
  }
  
  // Lọc theo thể loại (nếu không phải "Tất cả")
  if (criteria.genres && criteria.genres.length > 0 && !criteria.genres.includes('all')) {
    filteredMovies = filteredMovies.filter(movie => {
