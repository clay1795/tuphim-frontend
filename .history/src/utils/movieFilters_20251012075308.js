...
export const createDetailedMovieSections = async () => {
  ...
  // Tạo sections bằng cách gọi API cho từng section
  for (const config of sectionConfigs) {
    console.log(`\n--- Creating Section: ${config.title} ---`);
    console.log('Filter criteria:', config.filterCriteria);
    
    // Gọi API để lấy phim cho section này
    const sectionMovies = await fetchMoviesForSection(config.filterCriteria);
    
    console.log(`Movies fetched for "${config.title}": ${sectionMovies.length}`);
    if (sectionMovies.length > 0) {
      console.log('Sample movies:', sectionMovies.slice(0, 3).map(m => `${m.name} (${m.year})`));
    }
    
    // Thêm section với phim từ API
    sections.push({
      ...config,
