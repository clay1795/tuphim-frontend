...
const countryMapping = {
  'mỹ': ['usa', 'america', 'united states', 'us'],
  'anh': ['uk', 'england', 'britain', 'united kingdom'],
  'thái lan': ['thailand', 'thai', 'thái'],
  'trung quốc': ['china', 'chinese', 'trung quốc'],
  'nhật bản': ['japan', 'japanese', 'nhật'],
  'hàn quốc': ['korea', 'korean', 'hàn'],
  'đài loan': ['taiwan', 'taiwanese'],
  'hồng kông': ['hong kong', 'hongkong']
};

const genreMapping = {
  'hoạt hình': ['animation', 'anime', 'cartoon', 'hoạt hình'],
  'kinh dị': ['horror', 'thriller', 'scary', 'kinh dị'],
  'hành động': ['action', 'hành động'],
  'tình cảm': ['romance', 'love', 'tình cảm'],
  'hài hước': ['comedy', 'funny', 'hài'],
  'phiêu lưu': ['adventure', 'phiêu lưu'],
  'khoa học viễn tưởng': ['sci-fi', 'science fiction', 'khoa học'],
  'gia đình': ['family', 'gia đình'],
  'tài liệu': ['documentary', 'tài liệu']
};
...
export const fetchAllMovies = async () => {
  try {
    ...
    // Sử dụng search endpoint với keyword rộng để lấy tất cả phim
    const searchEndpoints = [
      'http://localhost:3001/api/movies/search?keyword=a&limit=50000',
      'http://localhost:3001/api/movies/search?keyword=e&limit=50000',
      'http://localhost:3001/api/movies/search?keyword=i&limit=50000'
    ];
    
    let allMovies = [];
    
    // Thử search endpoint đầu tiên
    try {
      console.log(`Trying search endpoint: ${searchEndpoints[0]}`);
      const response = await fetch(searchEndpoints[0]);
      
      if (response.ok) {
        const data = await response.json();
        console.log('Search API response:', data);
        
        // Xử lý dữ liệu từ search API
        if (data.success && data.data && data.data.items && Array.isArray(data.data.items)) {
          allMovies = data.data.items;
          console.log(`Found ${allMovies.length} movies from search API`);
          
          // Kiểm tra tổng số phim có sẵn
          if (data.params && data.params.pagination && data.params.pagination.totalItems) {
            console.log(`Total movies available: ${data.params.pagination.totalItems}`);
          }
        }
      }
    } catch (error) {
      console.log(`Error with search endpoint:`, error.message);
    }
    
    // Nếu chưa có đủ phim, thử fetch thêm từ các endpoint khác
    if (allMovies.length < 10000) {
      console.log('Fetching additional movies from other endpoints...');
      
      const additionalEndpoints = [
        'http://localhost:3001/api/movies/new?page=1&limit=10000&version=v3',
        'http://localhost:3001/api/movies/new?page=2&limit=10000&version=v3',
        'http://localhost:3001/api/movies/new?page=3&limit=10000&version=v3'
      ];
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.success && data.data && data.data.items && Array.isArray(data.data.items)) {
              ...
              additionalMovies = data.items;
            } else if (data.data && Array.isArray(data.data)) {
              additionalMovies = data.data;
            } else if (Array.isArray(data)) {
              additionalMovies = data;
            }
...
export const createDetailedMovieSections = async () => {
  ...
  for (const config of sectionConfigs) {
    ...
    let filteredMovies = filterMoviesByDetailedCriteria(allMovies, config.filterCriteria)
      ...
      .slice(0, 12); // Lấy tối đa 12 phim
    
    console.log(`Movies found for "${config.title}": ${filteredMovies.length}`);
    
    // Nếu không tìm được phim, thử fallback với bộ lọc linh hoạt hơn
    if (filteredMovies.length === 0) {
      console.log(`No movies found for "${config.title}", trying fallback...`);
      ...
      filteredMovies = filterMoviesByDetailedCriteria(allMovies, fallbackCriteria1)
        .filter(movie => {
          const movieId = movie._id || movie.id || movie.slug || movie.name;
          if (usedMovieIds.has(movieId)) {
            return false;
      ...
      console.log(`Fallback 1 movies found: ${filteredMovies.length}`);
    }
    
    // Nếu vẫn không tìm được phim, thử fallback với bộ lọc rất linh hoạt
    if (filteredMovies.length === 0) {
      console.log(`Still no movies found for "${config.title}", trying very flexible fallback...`);
      ...
      filteredMovies = filterMoviesByDetailedCriteria(allMovies, fallbackCriteria2)
        .filter(movie => {
          const movieId = movie._id || movie.id || movie.slug || movie.name;
          if (usedMovieIds.has(movieId)) {
            return false;
      ...
      console.log(`Fallback 2 movies found: ${filteredMovies.length}`);
    }
    
    // Nếu vẫn không tìm được phim, lấy phim ngẫu nhiên
    if (filteredMovies.length === 0) {
      console.log(`Still no movies found for "${config.title}", using random movies...`);
      ...
      filteredMovies = remainingMovies.slice(0, Math.min(4, 12));
      ...
      console.log(`Random movies found: ${filteredMovies.length}`);
    }
