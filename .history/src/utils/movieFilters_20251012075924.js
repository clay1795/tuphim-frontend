...
export const fetchAllMovies = async () => {
  try {
    ...
    // Thử nhiều endpoint để lấy tất cả phim
    const endpoints = [
      'http://localhost:3001/api/movies/all', // Endpoint cho tất cả phim
      'http://localhost:3001/api/movies?limit=50000', // Với limit cao
      'http://localhost:3001/api/movies/new?limit=50000&version=v3', // Với limit cao
      'http://localhost:3001/api/movies/new?page=1&limit=50000&version=v3' // Với limit cao
    ];
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (movieData.length > allMovies.length) {
          allMovies = movieData;
          console.log(`Updated with ${movieData.length} movies from ${endpoint}`);
        }
        
        // Nếu đã có đủ phim (>1000), dừng lại
        if (allMovies.length > 1000) {
          console.log(`Found ${allMovies.length} movies, stopping search`);
          break;
        }
    ...
    // Nếu vẫn chưa có phim, thử fetch từng trang
    if (allMovies.length < 1000) {
      console.log('Fetching movies page by page...');
      let page = 1;
      let hasMore = true;
      
      while (hasMore && allMovies.length < 50000) {
        try {
          const response = await fetch(`http://localhost:3001/api/movies/new?page=${page}&limit=100&version=v3`);
          ...
          if (data.success && data.data && data.data.items && Array.isArray(data.data.items)) {
            pageMovies = data.data.items;
          } else if (data.items && Array.isArray(data.items)) {
            pageMovies = data.items;
          } else if (data.data && Array.isArray(data.data)) {
            pageMovies = data.data;
          } else if (Array.isArray(data)) {
            pageMovies = data;
          }
          
          if (pageMovies.length === 0) {
            console.log(`Page ${page} has no movies, stopping`);
            hasMore = false;
            break;
          }
          
          allMovies = [...allMovies, ...pageMovies];
          console.log(`Page ${page}: ${pageMovies.length} movies (Total: ${allMovies.length})`);
          
          page++;
          
          // Dừng nếu đã có đủ phim
          if (allMovies.length >= 50000) {
            console.log(`Reached limit of ${allMovies.length} movies`);
            break;
          }
