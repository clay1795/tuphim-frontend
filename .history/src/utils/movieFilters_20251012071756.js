...
export const createUniqueMovieSections = (movies) => {
  ...
  // Section 1: Phim mới nhất
  const newMovies = filterNewMovies(movies);
  if (newMovies.length > 0) {
    const uniqueNewMovies = newMovies.filter(movie => !usedMovies.has(movie._id));
    if (uniqueNewMovies.length > 0) {
      sections.push({
        key: 'new-movies',
        title: 'Phim Mới Nhất',
        description: 'Những bộ phim mới nhất được cập nhật',
        movies: uniqueNewMovies.slice(0, 20),
        linkTo: '/movies/new',
        component: 'MovieSection'
      });
      uniqueNewMovies.slice(0, 20).forEach(movie => usedMovies.add(movie._id));
    }
  }
  
  // Section 2: Phim chất lượng cao
  const highQualityMovies = filterHighQualityMovies(movies);
  if (highQualityMovies.length > 0) {
    const uniqueQualityMovies = highQualityMovies.filter(movie => !usedMovies.has(movie._id));
    if (uniqueQualityMovies.length > 0) {
      sections.push({
        key: 'high-quality',
        title: 'Phim Chất Lượng Cao',
        description: 'Những bộ phim có chất lượng tốt nhất',
        movies: uniqueQualityMovies.slice(0, 20),
        linkTo: '/movies/quality',
        component: 'MovieSection'
      });
      uniqueQualityMovies.slice(0, 20).forEach(movie => usedMovies.add(movie._id));
    }
  }
  
  // Section 3: Phim Trung Quốc
  const chineseMovies = filterMoviesByTheme(movies, 'chinese');
  if (chineseMovies.length > 0) {
    const uniqueChineseMovies = chineseMovies.filter(movie => !usedMovies.has(movie._id));
    if (uniqueChineseMovies.length > 0) {
      sections.push({
        key: 'chinese-movies',
        title: 'Phim Trung Quốc',
        description: 'Những bộ phim Trung Quốc hay nhất',
        movies: uniqueChineseMovies.slice(0, 20),
        linkTo: '/movies/chinese',
        component: 'MovieSection'
      });
      uniqueChineseMovies.slice(0, 20).forEach(movie => usedMovies.add(movie._id));
    }
  }
  
  // Section 4: Phim Hàn Quốc
  const koreanMovies = filterMoviesByTheme(movies, 'korean');
  if (koreanMovies.length > 0) {
    const uniqueKoreanMovies = koreanMovies.filter(movie => !usedMovies.has(movie._id));
    if (uniqueKoreanMovies.length > 0) {
      sections.push({
        key: 'korean-movies',
        title: 'Phim Hàn Quốc',
        description: 'Những bộ phim Hàn Quốc nổi bật',
        movies: uniqueKoreanMovies.slice(0, 20),
        linkTo: '/movies/korean',
        component: 'MovieSection'
      });
      uniqueKoreanMovies.slice(0, 20).forEach(movie => usedMovies.add(movie._id));
    }
  }
  
  // Section 5: Phim Nhật Bản
  const japaneseMovies = filterMoviesByTheme(movies, 'japanese');
  if (japaneseMovies.length > 0) {
    const uniqueJapaneseMovies = japaneseMovies.filter(movie => !usedMovies.has(movie._id));
    if (uniqueJapaneseMovies.length > 0) {
      sections.push({
        key: 'japanese-movies',
        title: 'Phim Nhật Bản',
        description: 'Những bộ phim Nhật Bản đặc sắc',
        movies: uniqueJapaneseMovies.slice(0, 20),
        linkTo: '/movies/japanese',
        component: 'MovieSection'
      });
      uniqueJapaneseMovies.slice(0, 20).forEach(movie => usedMovies.add(movie._id));
    }
  }
  ...
  if (animeMovies.length > 0) {
    const uniqueAnimeMovies = animeMovies.filter(movie => !usedMovies.has(movie._id));
    if (uniqueAnimeMovies.length > 0) {
      sections.push({
        key: 'anime-movies',
        title: 'Anime',
        description: 'Những bộ anime hay nhất',
        movies: uniqueAnimeMovies.slice(0, 20),
        linkTo: '/movies/anime',
        component: 'MovieSection'
      });
      uniqueAnimeMovies.slice(0, 20).forEach(movie => usedMovies.add(movie._id));
    }
  }
  
  // Section 7: Phim Tình Cảm
  const romanceMovies = filterMoviesByTheme(movies, 'romance');
  if (romanceMovies.length > 0) {
    const uniqueRomanceMovies = romanceMovies.filter(movie => !usedMovies.has(movie._id));
    if (uniqueRomanceMovies.length > 0) {
      sections.push({
        key: 'romance-movies',
        title: 'Phim Tình Cảm',
        description: 'Những bộ phim tình cảm lãng mạn',
        movies: uniqueRomanceMovies.slice(0, 20),
        linkTo: '/movies/romance',
        component: 'MovieSection'
      });
      uniqueRomanceMovies.slice(0, 20).forEach(movie => usedMovies.add(movie._id));
    }
  }
