      ...
      return criteria.countries.some(country => {
        ...
        return movieCountries.some(movieCountry => {
          ...
          return movieCountry.includes(countryLower) ||
                 ...
                 (countryLower === 'mỹ' && (movieCountry.includes('usa') || movieCountry.includes('america'))) ||
                 (countryLower === 'anh' && (movieCountry.includes('uk') || movieCountry.includes('england') || movieCountry.includes('britain'))) ||
                 (countryLower === 'thái lan' && (movieCountry.includes('thailand') || movieCountry.includes('thai'))) ||
                 (countryLower === 'trung quốc' && (movieCountry.includes('china') || movieCountry.includes('chinese'))) ||
                 (countryLower === 'nhật bản' && (movieCountry.includes('japan') || movieCountry.includes('japanese')));
        });
      });
    });
  }
      ...
      return criteria.genres.some(genre => {
        ...
        return movieGenres.some(movieGenre => {
          ...
          return movieGenre.includes(genreLower) ||
                 ...
                 // Thêm một số từ khóa phổ biến
                 (genreLower === 'hoạt hình' && (movieGenre.includes('animation') || movieGenre.includes('anime') || movieGenre.includes('cartoon'))) ||
                 (genreLower === 'kinh dị' && (movieGenre.includes('horror') || movieGenre.includes('thriller') || movieGenre.includes('scary')));
        });
      });
    });
  }
...
export const createDetailedMovieSections = (movies) => {
  ...
  sectionConfigs.forEach(config => {
    let filteredMovies = filterMoviesByDetailedCriteria(movies, config.filterCriteria)
      .filter(movie => {
      ...
      .slice(0, 12); // Lấy 12 phim mỗi section
    
    // Nếu không đủ phim, thử fallback với bộ lọc linh hoạt hơn
    ...
    // Nếu vẫn không đủ phim, lấy phim từ tất cả phim còn lại (không theo bộ lọc)
    if (filteredMovies.length < 6) {
    ...
    // Luôn thêm section, ngay cả khi không có phim
    sections.push({
      ...config,
    ...
    // Debug logging
    console.log(`Section "${config.title}": ${filteredMovies.length} movies`);
  });
