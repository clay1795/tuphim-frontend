...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    // Allow VS Code Port Forwarding domains
    if (origin && (
      ...
      origin.includes('.vscode-remote') ||
      origin.includes('.tunnel') ||
      origin.includes('.forwarded')
    )) {
...
async function startServer() {
  try {
    ...
    app.listen(PORT, '0.0.0.0', () => {
      ...
      logger.info(`🌍 Server accessible from: 0.0.0.0:${PORT} (for port forwarding)`);
    });
  } catch (error) {
