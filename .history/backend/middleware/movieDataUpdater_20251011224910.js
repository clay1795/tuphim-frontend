...
class MovieDataUpdater {
  ...
  static async updateMovieData(movieData) {
    try {
      ...
      // Check if we need to update English name
      const needsEnglishNameUpdate = !movieData.originalName || 
                                      movieData.originalName === movieData.movieName ||
                                      movieData.originalName.trim() === '';

      // If already has real data and English name, return as is
      if (movieData.poster_url && movieData.poster_url.includes('phimimg.com') && !needsEnglishNameUpdate) {
        return movieData;
      }
