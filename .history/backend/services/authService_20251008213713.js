...
class AuthService {
  ...
  async addToFavorites(userId, movieData) {
    try {
      ...
      // Update movie data with real data from KKPhim API
      logger.info(`Updating movie data for: ${movieData.movieName} (${movieData.movieSlug})`);
      logger.info(`MovieDataUpdater available: ${typeof MovieDataUpdater.updateMovieData}`);
      ...
      try {
        ...
        logger.info(`Original input originalName: ${movieData.originalName}`);
        logger.info(`Final output originalName: ${updatedMovieData.originalName}`);
      } catch (updateError) {
        ...
        logger.info(`Using fallback data - Name: ${updatedMovieData.movieName}, English: ${updatedMovieData.originalName || 'Not set'}`);
      }
