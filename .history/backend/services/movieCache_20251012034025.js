...
class MovieCache {
  ...
  async fullUpdate() {
    ...
    try {
      ...
      let page = 1;
      let hasMore = true;
      const batchSize = 20; // Fetch 20 pages in parallel
      
      while (hasMore && page <= 1000) { // Limit to 1000 pages for safety
        console.log(`📡 Fetching pages ${page}-${Math.min(page + batchSize - 1, 1000)}...`);
        
        // Create batch of pages to fetch
        const pages = [];
        for (let i = page; i < Math.min(page + batchSize, 1001); i++) {
          pages.push(i);
        }
        ...
        batchResults.forEach(({ page, data, success }) => {
          // Fix: API returns data.data directly, not data.data.data
          if (success && data && data.data && data.data.items && Array.isArray(data.data.items)) {
            batchMovies = batchMovies.concat(data.data.items);
            totalPages = data.data.pagination?.totalPages || 1;
            console.log(`✅ Page ${page}: ${data.data.items.length} movies`);
          } else {
        ...
        console.log(`📊 Progress: ${allMovies.length} movies fetched`);
      }
