...
class AuthService {
  ...
  async updateProfile(userId, updateData) {
    try {
      const allowedUpdates = ['fullName', 'username', 'avatar', 'gender', 'preferences'];
      const updates = {};
  ...
  async addToWatchlist(userId, movieData) {
    try {
      ...
      // Add to watchlist
      user.preferences.watchlist.push(updatedMovieData);

      await user.save();

      logger.info(`Movie added to watchlist: ${updatedMovieData.movieId} for user: ${user.email}`);
  ...
  async addToFavorites(userId, movieData) {
    try {
      ...
      logger.info(`Input movie data: ${JSON.stringify(movieData)}`);
      logger.info(`MovieDataUpdater type: ${typeof MovieDataUpdater}`);
      logger.info(`MovieDataUpdater.updateMovieData type: ${typeof MovieDataUpdater.updateMovieData}`);
      
      let updatedMovieData;
      try {
        logger.info(`Calling MovieDataUpdater.updateMovieData...`);
        updatedMovieData = await MovieDataUpdater.updateMovieData({
          ...movieData,
          addedAt: movieData.addedAt || new Date()
        });
        
        logger.info(`========== MOVIEDATA UPDATER RESULT ==========`);
        logger.info(`Updated movie data: ${JSON.stringify(updatedMovieData)}`);
        logger.info(`Vietnamese name: ${updatedMovieData.movieName}`);
        logger.info(`English name: ${updatedMovieData.originalName || 'NOT SET'}`);
        logger.info(`==============================================`);
      } catch (updateError) {
        logger.error(`========== MOVIEDATA UPDATER ERROR ==========`);
        ...
        logger.info(`Using fallback data - English: ${updatedMovieData.originalName || 'Not set'}`);
      }
      ...
      logger.info(`Movie added to favorites: ${updatedMovieData.movieId} for user: ${user.email}`);
