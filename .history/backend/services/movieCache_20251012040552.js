...
class MovieCache {
  ...
  async fullUpdate() {
    ...
    try {
      ...
      let page = 1;
      let hasMore = true;
      const batchSize = 100; // Maximum batch size for fastest fetching
      
      while (hasMore && page <= 3000) { // Increased limit to fetch all movies like KKPhim
        console.log(`📡 Fetching pages ${page}-${Math.min(page + batchSize - 1, 3000)}...`);
        
        // Create batch of pages to fetch
        const pages = [];
        for (let i = page; i < Math.min(page + batchSize, 3001); i++) {
          pages.push(i);
        }
        
        // Fetch all pages in batch in parallel from multiple endpoints
        const endpoints = [
          '/danh-sach/phim-moi-cap-nhat-v3',
          '/danh-sach/phim-bo',
          '/danh-sach/phim-le', 
          '/danh-sach/hoat-hinh',
          '/danh-sach/phim-han-quoc',
          '/danh-sach/phim-trung-quoc',
          '/danh-sach/phim-nhat-ban',
          '/danh-sach/phim-thai-lan'
        ];
        
        const batchPromises = pages.flatMap(currentPage => 
          endpoints.map(async (endpoint) => {
            try {
              const result = await kkphimApi.getMoviesFromEndpoint(endpoint, currentPage, 20);
              return { page: currentPage, endpoint, data: result, success: true };
            } catch (error) {
              console.error(`❌ Error fetching page ${currentPage} from ${endpoint}:`, error);
              return { page: currentPage, endpoint, data: null, success: false };
            }
          })
        );
        ...
        console.log(`📊 Progress: ${allMovies.length} movies fetched (Batch ${Math.ceil(page/batchSize)}/${Math.ceil(3000/batchSize)})`);
      }
