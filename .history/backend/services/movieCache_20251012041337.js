...
class MovieCache {
  ...
  async fullUpdate() {
    ...
    try {
      ...
      while (hasMore && page <= 5000) { // Maximum limit to fetch all movies like KKPhim
        console.log(`📡 Fetching pages ${page}-${Math.min(page + batchSize - 1, 5000)}...`);
        
        // Create batch of pages to fetch
        const pages = [];
        for (let i = page; i < Math.min(page + batchSize, 5001); i++) {
          pages.push(i);
        }
