...
const KKPHIM_CONFIG = {
  ...
  rateLimit: {
    ...
    requestsPerMinute: 500, // Increased for massive fetching
  },
  utils: {
...
class KKPhimApiService {
   ...
   * Get new movies
   */
  async getNewMovies(page = 1, version = 'v3', limit = 24) {
    const endpoint = KKPHIM_CONFIG.api.newMovies[version] || KKPHIM_CONFIG.api.newMovies.v3;
    // KKPhim API seems to have a max limit of 24, so we cap it
    const actualLimit = Math.min(limit, 24);
    return await this.makeRequest(endpoint, { page, limit: actualLimit });
  }
