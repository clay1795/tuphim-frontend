...
class SyncScheduler {
  ...
  start() {
    ...
    // Job 1: Chạy incremental sync mỗi giờ
    ...
    // Job 2: Chạy full sync mỗi ngày lúc 3:00 AM (optional)
    const dailyJob = cron.schedule('0 3 * * *', async () => {
      logger.info('🌅 Daily full sync scheduled (disabled by default)');
      // await this.runFullSync(); // Uncomment if needed
    }, {
      scheduled: false,
      timezone: 'Asia/Ho_Chi_Minh'
    });
