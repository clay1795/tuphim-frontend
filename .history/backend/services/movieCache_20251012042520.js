  ...
  async fullUpdate() {
    ...
    try {
      ...
      while (hasMore && page <= 2000) { // Limit to 2000 pages to avoid crash
        ...
        // Safety limit: stop at 15000 movies to avoid crash
        if (allMovies.length >= 15000) {
          console.log(`🛑 Safety limit reached: ${allMovies.length} movies. Stopping to avoid crash.`);
          hasMore = false;
          break;
        }
