...
class MongoMovieService {
  ...
  async getMoviesByCategory(category, page = 1, limit = 24) {
    return this.searchMovies({ category, page, limit });
  }

  // Get movies by country
  async getMoviesByCountry(country, page = 1, limit = 24) {
    return this.searchMovies({ country, page, limit });
  }

  // Get movies by year
  async getMoviesByYear(year, page = 1, limit = 24) {
    return this.searchMovies({ year, page, limit });
  }

  // Get movies by type
  async getMoviesByType(type, page = 1, limit = 24) {
    return this.searchMovies({ type, page, limit });
  }

  // Get new movies (recently updated)
  async getNewMovies(page = 1, limit = 24) {
