...
async function testMovieDataUpdaterWithRealApi() {
  try {
    console.log('Testing MovieDataUpdater with real API data...');
    
    // Test with the exact movie data that's failing
    const movieData = {
      movieId: 'test-real-api-001',
    ...
    if (updatedData.originalName === 'The Balloonist') {
      console.log('\n🎉 SUCCESS! MovieDataUpdater is working correctly with real API!');
    } else {
      console.log('\n❌ ISSUE: MovieDataUpdater is not working correctly with real API');
...
testMovieDataUpdaterWithRealApi();
