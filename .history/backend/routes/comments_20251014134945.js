...
router.get('/:movieSlug', async (req, res) => {
  try {
    ...
    // Get top-level comments (not replies) with populated user data
    ...
    // Update usernames from populated user data
    const commentsWithUpdatedNames = comments.map(comment => {
      if (comment.userId && typeof comment.userId === 'object' && comment.userId.fullName) {
        comment.username = comment.userId.fullName;
        comment.avatar = comment.userId.avatar || comment.avatar;
      } else if (comment.userId && typeof comment.userId === 'string') {
        // For old comments with string userId, keep original username
        // This will be fixed when new comments are created
      }
      return comment;
    });
    ...
    const commentsWithReplies = await Promise.all(
      commentsWithUpdatedNames.map(async (comment) => {
        ...
        // Update usernames in replies
        const repliesWithUpdatedNames = replies.map(reply => {
          if (reply.userId && typeof reply.userId === 'object' && reply.userId.fullName) {
            reply.username = reply.userId.fullName;
            reply.avatar = reply.userId.avatar || reply.avatar;
          } else if (reply.userId && typeof reply.userId === 'string') {
            // For old comments with string userId, keep original username
          }
          return reply;
        });
        ...
        return {
          ...comment,
          replies: repliesWithUpdatedNames,
...
router.get('/top/:movieSlug', async (req, res) => {
  try {
    ...
    // Get top comments by votes and recency with populated user data
    ...
    // Update usernames from populated user data
    const topCommentsWithUpdatedNames = topComments.map(comment => {
    ...
    res.json({
      success: true,
      data: topCommentsWithUpdatedNames
    });
  } catch (error) {
