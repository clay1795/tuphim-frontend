  ...
  try {
    ...
    try {
      ...
      if (emailResult.success) {
        ...
        res.json({
          success: true,
          message: 'Yêu cầu đặt lại mật khẩu đã được tạo. Vui lòng kiểm tra email của bạn.',
          resetToken: resetToken // Fallback: return token if email fails
        });
      }
    } catch (emailError) {
      logger.error(`Email service error: ${emailError.message}`);
      res.json({
        success: true,
        message: 'Yêu cầu đặt lại mật khẩu đã được tạo. Vui lòng kiểm tra email của bạn.',
        resetToken: resetToken // Fallback: return token if email fails
