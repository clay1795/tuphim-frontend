...
/**
 * @route   GET /api/movies/cache/search
 * @desc    Search movies using cached data (instant results)
 * @access  Public
 */
router.get('/cache/search', async (req, res) => {
  try {
    const { 
      keyword = '', 
      page = 1, 
    ...
    logger.info(`Cache search - Keyword: "${keyword}", Type: "${type}", Page: ${page}, User: ${req.user?.email || 'anonymous'}`);

    // Search in cache
    const allResults = movieCache.searchMovies(keyword, {
      type,
      category,
      country,
      year,
    ...
    res.json({
      success: true,
      message: 'Cache search completed successfully',
      data: {
        ...
        cache: {
          totalMovies: movieCache.getStats().totalMovies,
          lastUpdate: movieCache.getStats().lastUpdate
        }
      }
    });
  } catch (error) {
    logger.error('Cache search error:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to search movies in cache',
      error: error.message
    });
  }
});
 ...
 * @desc    Force refresh cache (incremental)
 * @access  Public
 */
...
/**
 * @route   POST /api/movies/cache/full-update
 * @desc    Force full database update (all movies)
 * @access  Public
 */
router.post('/cache/full-update', async (req, res) => {
  try {
    logger.info('Manual full database update requested');
    
    // Start full update
    movieCache.fullUpdate();
    
    res.json({
      success: true,
      message: 'Full database update initiated - fetching all movies',
      data: movieCache.getStats()
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to start full update',
      error: error.message
    });
  }
});
...
router.get('/new', async (req, res) => {
  try {
    const { page = 1, version = 'v3' } = req.query;
    
    logger.info(`Fetching new movies - Page: ${page}, Version: ${version}, User: ${req.user?.email || 'anonymous'}`);

    const movies = await kkphimApi.getNewMovies(parseInt(page), version);
