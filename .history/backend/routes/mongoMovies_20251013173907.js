...
/**
 * @route   GET /api/mongo-movies/series/:slug
 * @desc    Get series details for a movie (all parts of the series)
 * @access  Public
 */
router.get('/series/:slug', async (req, res) => {
  try {
    ...
    logger.info(`Fetching series details for slug: ${slug}`);
    ...
    // Check if it's a series movie
    const isSeries = isSeriesMovie(currentMovie);
    ...
    if (!isSeries) {
      ...
      return res.json({
        ...
        data: {
          isSeries: false,
          currentMovie: transformMovie(currentMovie),
          allParts: [transformMovie(currentMovie)]
        }
      });
    }

    // Find all movies in the same series
    const baseName = getSeriesBaseName(currentMovie.name);
    const seriesMovies = await Movie.find({
      $or: [
        { name: { $regex: `^${baseName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*(phần|part|season|tập|episode|ep|s)\\s*\\d+`, $options: 'i' } },
        { name: { $regex: `^${baseName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*\\d+$`, $options: 'i' } },
        { name: { $regex: `^${baseName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*-\\s*\\d+`, $options: 'i' } }
      ]
    }).sort({ last_sync: -1 }).lean();
    ...
    const sortedMovies = seriesMovies
      ...
      .map(movie => ({
        ...movie,
        partNumber: getSeriesPartNumber(movie.name)
      }))
    ...
    res.json({
      ...
      data: {
        ...
        baseName,
        currentMovie: {
          ...transformMovie(currentMovie),
          partNumber: getSeriesPartNumber(currentMovie.name)
        },
        allParts: sortedMovies.map(transformMovie),
        totalParts: sortedMovies.length
      }
    });
  } catch (error) {
    logger.error('Series details error:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to get series details',
      error: error.message
    });
  }
});
