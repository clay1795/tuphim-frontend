  ...
  try {
    ...
    const commentsWithReplies = await Promise.all(
      commentsWithUpdatedNames.map(async (comment) => {
        ...
        // Update usernames in replies too
        const repliesWithUpdatedNames = await Promise.all(
          replies.map(async (reply) => {
            if (reply.userId) {
              try {
                const User = require('../models/User');
                const currentUser = await User.findById(reply.userId).select('fullName username').lean();
                if (currentUser && currentUser.fullName) {
                  reply.username = currentUser.fullName;
                }
              } catch (error) {
...
router.get('/top/:movieSlug', async (req, res) => {
  try {
    ...
    // Update usernames from current user data
    const topCommentsWithUpdatedNames = await Promise.all(
