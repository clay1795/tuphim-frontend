 ...
 * @desc    Get all movies from database (massive fetch for instant results)
 * @access  Public
 */
router.get('/all', async (req, res) => {
  try {
    ...
    logger.info(`Fetching all movies - Limit: ${limit}, Offset: ${offset}, User: ${req.user?.email || 'anonymous'}`);
    ...
    const pagesToFetch = Math.min(parseInt(limit) / 24, 50); // Max 50 pages to avoid timeout
    
    for (let page = 1; page <= pagesToFetch; page++) {
      try {
        const movies = await kkphimApi.getNewMovies(page, 'v3', 24);
        
        if (movies && movies.data && movies.data.items) {
          allMovies = allMovies.concat(movies.data.items);
          totalPages = movies.data.pagination?.totalPages || 1;
          totalItems = movies.data.pagination?.totalItems || allMovies.length;
        }
        ...
        logger.warn(`Error fetching page ${page}:`, pageError.message);
        break;
      }
    }
    ...
    res.json({
      success: true,
      message: `Retrieved ${allMovies.length} movies from ${pagesToFetch} pages`,
      data: {
        items: allMovies,
        pagination: {
    ...
    res.status(500).json({
      error: 'Failed to fetch all movies',
      message: 'Unable to retrieve all movies. Please try again later.'
    });
  }
});
