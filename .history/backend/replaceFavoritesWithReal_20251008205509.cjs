...
async function replaceFavoritesWithReal() {
  try {
    ...
    if (!newMovies.items || !newMovies.items.length) {
      console.log('❌ No movies found from API');
      return;
    }
    
    console.log(`✅ Found ${newMovies.items.length} real movies`);
    
    // Create new favorites list with real movies (take first 12)
    const realMovies = newMovies.items.slice(0, 12);
    const newFavorites = [];
