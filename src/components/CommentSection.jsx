              ...
              <div className="flex items-center space-x-2">
                {user ? (
                  <span className="text-gray-400 text-sm">
                    Đăng nhập với tên: <span className="text-yellow-400">{user.fullName || user.username || user.email}</span>
                  </span>
                ) : (
                ...
                <div key={comment._id} className="bg-gray-800 rounded-lg p-4">
                  <div className="flex items-start space-x-3">
                    <img
                      ...
                      onError={(e) => {
                        const displayName = comment.fullName || comment.username;
                        e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName.charAt(0))}&background=4B5563&color=fff&size=40`;
                      }}
                    />
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-2">
                        <span className="text-white font-semibold text-sm">{comment.fullName || comment.username}</span>
