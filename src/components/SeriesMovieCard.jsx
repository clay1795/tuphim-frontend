...
const SeriesMovieCard = ({ movie, index = 0 }) => {
  ...
  const handleMovieClick = () => {
    console.log('Series movie clicked:', movie.name, 'Slug:', movie.slug, 'ID:', movie._id);
    console.log('Series parts:', movie.seriesParts?.length || 0);
  };

  // Tính tổng số phần
  const totalParts = movie.seriesParts?.length || 1;
  const currentPart = movie.partNumber || 1;
  ...
  return (
    <div className="group animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
      ...
      <MovieTooltip movie={movie}>
        ...
        >
          <div className="relative overflow-hidden rounded-lg sm:rounded-xl shadow-xl sm:shadow-2xl hover:shadow-2xl sm:hover:shadow-3xl transition-all duration-300 sm:duration-500 group-hover:scale-102 sm:group-hover:scale-105 group-hover:-translate-y-1 sm:group-hover:-translate-y-2">
            <div className="relative aspect-[3/4] overflow-hidden">
              ...
              <img
                ...
                loading="lazy"
                onLoad={() => {
                  console.log('Image loaded successfully for:', movie.name, 'URL:', movieApi.getWebpImage(movie.poster_url || movie.thumb_url));
                }}
                onError={(e) => {
                  console.log('Image load error for:', movie.name, 'URL:', movieApi.getWebpImage(movie.poster_url || movie.thumb_url));
            ...
            {/* Series info badge - Hiển thị thông tin về series */}
            {totalParts > 1 && (
              <div className="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 z-10">
                <div className="bg-gradient-to-r from-blue-600/90 to-purple-600/90 backdrop-blur-md text-white px-2 sm:px-3 py-1 rounded-full text-xs font-bold border border-white/20 shadow-lg">
                  {totalParts} Phần
                </div>
              </div>
            )}
            
            {/* Episode info cho phần hiện tại */}
            {movie.episode_current && movie.episode_total && (
              <div className="absolute bottom-2 sm:bottom-3 right-2 sm:right-3 z-10">
                <div className="bg-black/80 backdrop-blur-md text-white px-2 sm:px-3 py-1 rounded-full text-xs font-bold border border-white/20 shadow-lg">
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500">
              <div className="relative">
                <div className="bg-red-600/90 backdrop-blur-sm rounded-full p-3 sm:p-4 hover:bg-red-700/90 transition-all duration-300 animate-pulse-glow shadow-xl sm:shadow-2xl">
        ...
        {/* Movie Title Section - Outside MovieTooltip but clickable */}
        <Link 
          to={`/phim/${movie.slug || movie._id}`}
        ...
        >
          ...
          {/* Series Status - Hiển thị thông tin về các phần */}
          ...
          {/* Episode Status cho phần hiện tại */}
          {movie.episode_current && movie.episode_total && (
            <div className="flex items-center space-x-2">
              <span className="text-xs text-gray-500">Tập:</span>
...
export default SeriesMovieCard;
